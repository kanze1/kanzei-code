...
| `defs.rs`          | `AgentDef`、`SkillDef`、profile 和 agent mode 等基础定义。                                                                            |
| `registry.rs`      | 五类注册表共用的“按名称、保持顺序、同名后注册覆盖”容器。                                                                                  |
...
| `markdown.rs`      | 扫描全局和项目的`agents/skills` Markdown 扩展。                                                                                           |
...
| `research_experiment_runner.md`                 | 实验运行、路线图投影与环境/预算的字段冻结。      |
