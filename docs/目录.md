...
这些 Markdown 才是需求、缺陷、目标和项目记忆的可编辑真源。SQLite 不应该反过来取代它们。
...
- `M-060`：并行线路合并 SOP。
- `M-061`：自举复盘与证据链检查。
- `M-062`：当前机器 WebView2 DevTools/CDP 的环境限制。
...
| `work.rs`         | 当前未跟踪、开发中的“需求/缺陷双队列确定性取活”工具；由代码决定 Resume/Start，避免模型自行解释两份列表。 |
...
| `06-activity.js`     | 工具活动流、diff 树、阶段、实时进度、恢复轨迹和过滤。                                            |
| `06-agent-panel.js`  | 子代理运行中/完成/关闭面板、usage 和 transcript。                                                |
...
这部分是上游 OpenCode 资料或移植参考，不是 kanzei 当前实现的正式契约。

- `readme.md`：明确 reference 的定位。
- `CONTEXT.md`：OpenCode Session Runtime 的大体上下文契约。
- `deepseek_harness_reference_20260814.md`：DeepSeek Harness 开源后对 Kanzei 的源码核对、差距分析与实施参考原文。
- `specs-v2/config.md`：配置模型。
- `specs-v2/provider-model.md`：Provider 和 Model Catalog。
- `specs-v2/provider-policy.md`：Provider 策略。
- `specs-v2/session.md`：Session API。
- `specs-v2/tools.md`：工具协议。
- `specs-v2/catalog-config-plugin-lifecycle.md`：Catalog、配置和插件生命周期。
- `opencode-archive/readme.md`：归档边界说明。
- `opencode-archive/tui-package.md`：上游 TUI 包拆分资料。
- `opencode-archive/specs-v2/instructions.md`：旧 V2 Instructions。
- `opencode-archive/specs-v2/schema-changelog.md`：旧 schema 变更记录。
- `opencode-archive/specs-v2/todo.md`：旧待办。

`docs/reference/src/` 中的 TypeScript：

- `anthropic-messages.ts`：Anthropic 协议参考实现。
- `openai-chat.ts`：OpenAI Chat 协议参考。
- `llm-events.ts`：统一事件参考。
- `proxy-env.ts`：代理环境变量语义参考。
- `shell.ts`：shell 检测参考。
- `compaction.ts`：压缩逻辑参考。
- `context-epoch.ts`：上下文 epoch 语义。
- `run-coordinator.ts`：运行协调参考。
- `runner-llm.ts`：Runner 与 LLM 连接参考。
- `session-input.ts`：输入 admission/steer/queue 参考。

这些 `.ts` 文件不参与 Cargo 或桌面前端构建。
...
| `e2e-smoke.mjs`                 | 尝试通过 WebView2/CDP 驱动真实 Tauri 桌面端。                                                                   |
| `probe-webview-cdp.mjs`         | 单独探测 WebView2 DevTools/CDP 连接。                                                                           |
...
| `r050-poc-check.ps1`            | R-050 对应 PoC 的定向验证脚本，不等于全量发布验证。                                                             |
...
- DeepSeek Responses：
  ...
  - `kanzei-harness/src/config.rs`
  - `kanzei-core/src/assemble.rs`
  - `kanzei-llm/src/client.rs`
  - `kanzei-llm/src/protocol/mod.rs`
  - `kanzei-llm/src/protocol/deepseek_responses.rs`，未跟踪
  - `kanzei-app/src/settings.rs`
  - `ui/16-settings.js`
- 记忆召回和工具契约：

  - `kanzei-core/src/runner/recall.rs`
  - `kanzei-harness/src/tool.rs`
  - `kanzei-tools/src/lib.rs`
  - `kanzei-tools/src/memory/mod.rs`
  - `kanzei-tools/src/profiles.rs`
- 确定性需求/缺陷取活：

  - `kanzei-tools/src/work.rs`，未跟踪
- 运行、权限和项目文档：

  - `kanzei-app/src/run.rs`
  - `.kanzei/kanzei.toml`
  - `.kanzei/project/defects.md`
  - `.kanzei/project/tests-archive.md`

而且在本次只读盘点期间，最终状态比第一次读取多出了几项修改，说明工作区可能有另一个进程或会话仍在写入。以上内容我只做了读取，没有改动。
