# 多进程代理编排：阶段流历史基线与任务级并行修订

- 状态：**历史基线，部分现行** —— 阶段流与只读子代理仍有效；不变量 3~5 的项目级单 writer/全工具串行已由 R-182 取代
...
以下是 2026-08-10 的历史调度原则：

> 并行查，串行写。

它曾被落实为项目级执行契约；2026-08-11 的机械实测证明该契约把已由 worktree 隔离的代码写入无谓串行化，现行替代契约见文首修订、R-182 与 conventions §10。
...
- `deep_parallel_dev.md`：现行任务级并行基线；不同 worktree 可以同时写各自代码树，文档与状态统一重定向主根。
...
- R-177/R-182 已在 `f052ca8`、`b1a2f98`、`c651096`、`fc44c38`、`ea93158`、`e98cead`、`e791536` 落地：真实 worktree 绑定、cwd/主根双键、git 清单、主根 tracker、跨树 OS 进程并发登记及四命令闭环均有自动化证据。
- R-184 的协作上下文与基础并列/冲突视图已在 `e3679a2`、`2d432fc` 落地；完整收活五格与按线设置仍是 TODO。

## TODO 与后续风险

1. R-171 已实施；协调器保留为显式收紧接口与单树/旁路操作仲裁，不再无条件覆盖不同 worktree 的整轮运行。
