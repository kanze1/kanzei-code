...
> **2026-08-21 修订(R-322 B2):「直接勾连跑,自动切」这条承诺撤回。**
> 本文促成的 R-224 让结伴档勾鞭挞自动切成 dev-auto。那条行为的**真实前提**是
> 当时结伴档根本不能续跑(`auto_allowed` 要求 `agent == "dev"`),所以「省去先切
> 模式再勾两步」实际上是「你要 loop 就得换掉人格」。R-322 B2 引入门禁强度后,
> 结伴档能以轻控制续跑(不 Nudge、不插核查轮、不标冗余、不拿 backlog 当停机判据),
> 前提消失。现在勾鞭挞就留在结伴档跑,人格不动;要重门禁请显式切 dev-auto。
> 本文其余关于双人格职责分工的结论**继续有效**。迁移/回滚边界:行为只在前端
> `08-compose.js` 的 auto-continue change 分支与 `coordinator.rs` 的 `auto_allowed`
> 判据两处,回滚即恢复 R-224 原分支。详见 `model_autonomy_and_harness_intensity.md`。
