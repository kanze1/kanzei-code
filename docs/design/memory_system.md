...
状态: 设计基线(存储形态仍有效) + 已交付实现对账(2026-08-18)
需求: R-103(总纲) → R-104/R-105/R-106/R-107(四期)；扩展交付：R-194(global scope 废弃) → R-195(candidate 生命周期) → R-213(provenance) → R-216(写入三闸) → R-233(hybrid 召回) → R-255(拆域) → R-286(控制面与 outcome 漏斗)

## 0. 定调(用户拍板的决策记录,后续不再重议)

- **文件优先**:非参数化外部记忆的核心优势就是可编辑与透明。真源永远是 markdown 文件,人可直接改,git 可完整恢复。
- **检索形态**:不接入外部向量库、知识图谱或 Mem0 类框架；项目内 `index.db` 可在显式 embedder 配置下提供 dense/hybrid 第二通道，无 embedder 时 lexical 仍完整可用(R-233/A-011)。
...
status: candidate       # candidate | shadow | active | deprecated | invalid；旧 stale 读入后归一为 deprecated
created: 2026-08-07
updated: 2026-08-08
...
- 删除 = 归档:`deprecated`或 `invalid` 条目由引擎自动搬运进 `archive/`(目录实际名;任何写操作后的 `refresh_derived` 都先跑 `archive_dead` 做 rename),带墓碑——墓碑 = 文件本身内容保留,失效 reason 追加进正文随文件进归档,绝不静默消失;ID 由归档侧保留永不复用。旧 `stale` 输入只归一为 `deprecated`。
...
- `memory_add(scope, category, title, description, content)` — 写入 candidate;引擎做近似去重与写入质量门禁。
- `memory_promote(id, sources)` — candidate/shadow 按真实 episode provenance 晋升 active;证据缺失或写入失败即拒绝。
- `memory_update(id, patch)` — 演化:内容修订、description 调整、命中后的强化;
- `memory_merge(ids, merged)` — 合并重复,被并条目自动 deprecated 并在墓碑里链到新条目;
- `memory_stale(id, reason)` — 失效标记,写入 deprecated/invalid 墓碑并归档；旧 stale 名称只作兼容输入;
- `memory_search(query, scope?, category?, status?)` — FTS5 BM25 + 可选 dense/hybrid 第二通道,结果按相关度与决策权重排序;
- `memory_stats()` — 各 scope/category 计数、召回/采纳漏斗、价值画像与 candidate 清退候选;
- `memory_inbox_clear` / `memory_inbox_discard` — 按批或按指纹销账 inbox,不静默清空。
...
| 空闲整理(手动,可关) | 桌面端「一键整理」按钮:零采纳候选或过期 candidate 按生命周期规则降级为 deprecated 并归档;积压进入整理清单可复查——非 sleep-time 自动 | 用户触发 |
...
- **条目视图**:正文、来源轨迹链接(source 字段可跳转)、hits、candidate/shadow/active/deprecated 状态与对应操作;直接编辑(写盘即生效);
...
- **R-107(M4 UI 与空闲整理)**:Memory 页(架构图/条目/账单/检索)+ 手动整理(一键降级零采纳候选或过期 candidate 并保留墓碑)+ 溢出压缩联动。验收:800/1024/1280 三档可用;整理动作全部有墓碑与日志,无静默删除。sleep-time 自动整理按设计偏差关闭,改手动按钮(R-132)。
