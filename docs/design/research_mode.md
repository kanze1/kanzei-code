...
> **2026-09-01 修订(A-016/A-018)**:定调点 6 的「bash 硬 deny」**不变**,但替代指引补一条——
> 实验命令走 **Experiment Runner 专用工具通道**。用户可以指定任意实验命令,但必须经运行器与
> 环境管理器执行,不能绕过这条链路直接起 bash;原指引只指向 latex/plot,漏了实验执行这一类。
> 详见 [research_experiment_runner.md](research_experiment_runner.md) §2.5/§8。
