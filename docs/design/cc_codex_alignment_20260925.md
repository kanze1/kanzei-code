...
1. 定时任务 `server` 档的理解:「托管到你登记的服务器,在服务器上定时跑远端 kz,结果拉回本地」。如果你指的是 webhook 或 GitHub 推送这类外部事件触发,R-370 的 B6 要改写。
2. R-245 的磁盘配额语义(上限多少,超限时拒绝外置还是降级为截断)仍待拍板,R-376 不含这一项。
