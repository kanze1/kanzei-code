...
1. 制作阶段烘焙透明边缘，输出上半 RGB、下半灰度 Alpha 的 H.264 视频。逻辑画幅 576 × 864，编码画幅 576 × 1728。v7.1 的嘴部与刺青改为干净底片，细节从高清参考图单独采样。
...
4. 画布最多按 1.5 倍像素比绘制，且不超过素材的有效分辨率；v7.1 的有效宽度包含 1024 像素高清嘴部来源。读取缓冲仅用于导出。
...
旧嘴型使用大块插值肤色覆盖原嘴，细线还会受到视频压缩影响。v7.1 用局部清理后的肤色底片，加上源图嘴部的笔触差值；张嘴范围限制在唇部，固定上唇位置向下展开。原图刺青独立采样，使用逐帧颈部仿射跟踪，避免视频生成重画图案。清理颈部时只使用暴露皮肤的样本，防止衣领颜色进入肤色底片。

逐帧检查同时发现 Pixi `VideoResource.updateFPS` 会在显式更新时再次限速，视频纹理可能滞后于跟踪位置。现在只由解码帧版本限制上传，关闭 Pixi 的第二次限速。导出定位落在目标帧的时间区间内，等待 `seeked` 后明确上传对应帧，避免 24 fps 时间戳舍入导致读取前一帧。连续播放跟随解码回调；暂停定位使用 `seeked`，因为 WebView2 在暂停时可能不向合成器提交视频帧，也就不会返回呈现回调。

36 个首、中、尾位置及 144 次嘴型开合通过像素检查：嘴部周围的变化像素为零，部分开口和完全开口均只有一个嘴部轮廓。另逐图检查全部动作的嘴部和原图刺青；记录保存在 `output/oc-v7-details/`。

## 2026-09-25 角色位置与外观设置

开启左侧「角色」后，可直接拖动角色，使用滚轮或右下角手柄缩放。选中角色后，方向键移动，Shift 加快移动，`+` / `-` 缩放，Home 恢复当前界面的默认位置和大小，Esc 取消正在进行的拖动。

「设置 → 角色外观与动作」提供大小（60%～180%）、不透明度（40%～100%）、完整动作 / 仅待机 / 静态立绘、锁定以及重置位置与大小。锁定后鼠标可穿过角色操作界面。三种动作模式均保留实际音频驱动的嘴型，并遵循系统的减少动态效果设置。

调整立即保存到本机，欢迎页、文字对话和语音界面分别保存位置。拖动按画面刷新合并，结束时写入存储；窗口缩小时将角色限制在可见区域内。CSS 负责移动缩放，渲染器按实际显示尺寸分配画布，画布的 CSS 尺寸保持填满原始容器，避免重复缩放造成裁切。

浏览器交互检查运行 `scripts/oc-layout-qa.cjs`，覆盖拖拽、滚轮、手柄、键盘、取消、跨界面位置、重载保存、静态嘴型、锁定、窗口边界和关闭释放。截图保存在 `output/oc-layout-settings/`。
