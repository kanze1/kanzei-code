...
状态：本机受控通信 POC 已实现；移动端交付形态已于 2026-08-16 拍板（见 §0），实施载体 R-270/R-271。

> **文档状态(2026-08-08 整理):进行中。** 对应 R-059 仍为活跃 todo;注意 `direction_taste.md` §5.6 冻结清单**建议 R-059 冻结或 dropped,待用户逐条确认**——若确认冻结,本文档同步标记为冻结。
...
- 不新增移动端框架；原生客户端属于后续工作。
- 不把 Tauri `kz:*` 事件直接暴露为远程 API。
- 不开放公网端口；当前 token 只服务本机桥接，停止服务即撤销。
- 远程权限回答、远程 shell/write 和公网部署不属于当前 POC。
