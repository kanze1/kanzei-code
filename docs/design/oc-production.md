...
透明边缘和闭嘴画面在制作阶段烘焙，保留手臂、颈部的实色描边并清除孤立背景点。播放器采样视频的颜色与透明平面，张嘴时才在口部局部区域覆盖嘴型。跟踪数据与真实解码帧时间对应。技术调查、实现和性能证据见[播放方案](oc-playback.md)。
...
v7 的 11 个运行视频合计 13,543,962 字节。v6 的 28 份运行文件已移入 `C:/Users/kanzei/Documents/kanzei-oc-archive/2026-09-24-v6-runtime/`，逐项核对 SHA256 后退出软件资源目录。H3 服务已停止，GPU 4、5 各恢复到 4 MiB，30010 端口关闭。
