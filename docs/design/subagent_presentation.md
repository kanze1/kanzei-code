...
- **宽度**:默认 `clamp(360, 26vw, 520)`;左缘分隔条(00-frame.js 的 `installSplit`,ui_surface_stack §4.6)可拖,停靠时范围 `[320, min(760, 主区宽 − 600)]`,偏好写进 `ui_layout.splits.tasks`;头部 ⤢ 在默认宽与当前形态的上限之间切换。
- **抽屉**:停靠后对话列不足 600px(`主区宽 − 侧栏宽 < 600`,即主区不到 920px)时改为抽屉:`data-dock="drawer"`,贴 `#main` 右缘绝对定位,带阴影,下面铺一层遮罩 `#tasks-scrim.k-scrim`(点遮罩收起)。抽屉宽度有自己的口径:默认 400,用户拖过的宽度夹到 `[320, 主区宽 − 96]`(停靠口径的上限在抽屉态几乎总是 320,拿它夹抽屉就拖不动;CSS 另有 `min(侧栏宽, 100% − 96px)` 兜底)。抽屉只由用户打开,自动开合只亮徽标(§7)。
- **三段**:
...
- **委派卡**(一批一张,Claude 桌面端「后台任务」卡片的骨架):
  ...
  - 小表:「子代理 · 模型 · Token · 用时」(英文界面按 Claude 写作「Agent · Model · Tokens · Time」,卡片类别同为「Agent」)一行一个子代理,点行进入详情。模型列只显示型号(`provider:型号` 去掉 provider,完整 id 在 title 里);侧栏不宽于 440px(容器查询:1280@1.5 的停靠宽 352/360、1600@1.25 的默认宽 416、抽屉 320)时隐去模型列,子代理列拿回宽度、描述整段可读(Claude 的表里 Agent 列是完整的),模型仍在子代理列的 title、行的可访问名与详情里;2000@1 的默认宽 520、或用户拖宽/⤢ 之后保留四列。行的可访问名只随状态变化重写,1 秒刷新只改用时文本。
...
| 规则 | 口径 |
|---|---|
| 自动打开 | 活动线路上**实时**开始了一个子代理,或一条终端命令跑满 **3 秒**;历史回放、后台线路(包括在后台开始、用户切过去时仍在跑的子代理)、编排 task 的终端重复条目都不触发。 |
| 不抢焦点 | 自动打开时焦点留在原处(用户可能正在打字),对话列宽度变化时保持「跟随最新」或原阅读位置不跳。 |
| 本次运行内的压制 | 「本次运行」= 用户手动发出一条消息到下一条用户消息;鞭挞自动续跑的轮次属于同一次运行。本次运行里用户关过侧栏,就不再自动打开,直到下一条用户消息——不论关的那一刻还有没有活:延迟收起期间关、失败保留期间关(为了确认失败只能关),正是鞭挞循环里最常见的时刻。 |
| 自动收起 | 由自动打开的侧栏在活动线路**全部结束 6 秒后**收起;指针悬停或焦点在侧栏内时暂停计时,离开后重新计时。用户手动打开的(rail、↗、命令面板)不自动收起。不在前台的线路子代理全部结束时记下时刻(策略事件 `idle`),切回来时已过 6 秒就直接退出自动态,不空弹;后台线路的终端命令不在册,切进一条已经停下的线路按已到点处理。 |
...
- **侧栏焦点**:`#tasks-panel` 是 `<aside>` 地标(`aria-labelledby` 指向标题),不是对话框,不设焦点陷阱。进入详情时焦点落在「‹ 返回」,返回列表后落在 ✕;Esc 在详情模式回到列表,在列表模式关闭侧栏(算用户关闭,§7.1),并把焦点还给打开它的元素(没有就还给 rail 开关)。Esc 监听挂在 `#tasks-panel` 自身,不挂 document/window(ui_surface_stack §5 禁令);弹层里的 Esc(「筛选与清理」菜单及其中的原生下拉:按键目标在 `[popover]` 里,或下拉列表开着)一律交给 00-surface 与浏览器,不算关闭侧栏——下拉列表开着时 00-surface 有意放行,冒泡上来若当成关闭,会确认失败、压制本次运行,还把菜单孤立在隐藏的侧栏里。自动打开从不移动焦点;用户手动打开的抽屉把焦点放进侧栏头部。
- **小表的行**:`<div role="listitem">` 里包一个原生 button(listitem 不能挂在 button 上,会盖掉按钮语义,读屏不知道它能点开详情)。可访问名「{人格 · 描述} — {状态与计数} — {型号}」,状态词只出现一次:终态用 `subagentMetaText`(它以状态词开头),运行中只写状态词。
- **分隔条**:手柄的提示与读屏名带 `data-i18n-title`/`data-i18n-aria-label`,运行中切语言跟着重译。
- **rail 开关**:`#tasks-toggle` 带 `aria-controls="tasks-panel"` 与 `aria-expanded`;徽标数字同时写进开关的可访问名(「N 运行中」/「N 个失败待查看」,英文按数量选单复数),不只靠颜色。
...
- **只有开始、没有结束的终端调用**:线路还在跑时,只有**最新一次运行**(最后一个 payload 的 `run_id`)里的它们是正在跑的调用,保留运行态、等真实的收尾事件;更早运行里悬空的(崩溃、停止时没补写 completed)与线路已停时的一律收成「中断」,已确认、落进已完成,不带停止按钮、不计在跑数、不挡自动收起。回放途中到达的实时调用(不是这次回放建的条目)按最新一次运行算。
...
| 冒烟:自动开合策略 15 个场景、集成(自动打开不抢焦点、6 秒收起、失败红徽标、切视图/切线路)、缺陷 A–F 回归、设置持久化;变异 `sideSuppressRun`/`sideFailureHold`/`sideLinger`/`sideLineScope`/`sideReplayPlace`/`sideReplayRunning`/`sideViewGate`/`sideNoFocusSteal`;复核修复(延迟收起中/失败保留中关闭也压制、后台线路不空弹、弹层 Esc、缺陷 B 收尾一侧、回放不保留失败、阅读位置、偏好合并、缺陷 F 按 run_id、抽屉宽度、分隔条重译、行结构与可访问名、英文「Agent」、型号去前缀)的变异 `sideBgStartScope`/`sideBgIdle`/`sideSwitchInIdle`/`sideEscPopover`/`sideBgEndScope`/`sideReplayNoHold`/`sideScrollAnchor`/`layoutAdoptPending`/`sideReplayRunScope`/`sideDrawerWidth`/`splitI18nKeys`/`sideRowListitem`/`sideRowAriaOnce`/`sideAgentWord`/`sideModelShort`;窄窗口停靠/抽屉判据 | `scripts/ui-runtime-smoke.mjs`「分区:后台任务侧栏与可调框」、`scripts/ui-narrow-layout-smoke.mjs` |
| 浏览器冒烟:侧栏停靠时权限卡与输入区同宽、不压侧栏;芯片让开侧栏;弹层里的 Esc 只关栈顶;窄侧栏隐去模型列;`page.route` 变异 `askComposerAnchor`/`chipDockRight`/`escPopoverBrowser`/`tasksNarrowModel` | `scripts/ui-surface-gallery-smoke.mjs` 步骤 8(ui_surface_stack §7.3) |
...
9. 本次运行里手动关掉侧栏后(含全部结束后的 6 秒延迟收起期间、失败保留期间),鞭挞续轮再派发子代理不再弹出;发下一条消息后恢复自动弹出。切到一条子代理早已在后台跑完的线路,侧栏不空弹。
...
- 2026-09-26 复核修复(UI2-0926 #14#4):§7.1 压制口径去掉「(且当时还有活)」——延迟收起中、失败保留中关掉也压制到下一条用户消息;后台线路上开始的子代理不触发自动打开,不在前台的线路活结束时记下时刻(`idle`),切回时不空弹 6 秒;侧栏 Esc 跳过弹层内的按键;抽屉宽度按自己的口径(默认 400、上限主区 − 96);缺陷 F 只保留最新一次运行里悬空的调用;小表窄侧栏隐去模型列、型号去 provider 前缀、英文表头「Agent」;行改为 listitem 包按钮、可访问名只拼一次状态词;分隔条切语言重译(§5.6、§7.1、§8、§11.2、§11.3、§15)。
