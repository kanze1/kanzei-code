...
- 日期: 2026-08-11
- 状态: **本轮发布范围已执行完成** —— D-267/F0/F2/F5 已按用户定调作废；D-269、F4/F6/F7/F9/F11/F13、R-182 内容①与 R-184 基础 A/B 面已交付。F10/F12 无人值守 CLI 与 R-184 增强 UI 不在本次发布范围
...
## §0 定案(2026-08-11 用户逐条拍板,实施时以本节为准)

### 定案 1 · 无人值守的安全线:**接受「不假装 + 收窄」**

对抗证伪指出:段级闸门是纯 shell 语法过滤器,对「被允许的**程序**本身是什么」一无所知。
`cargo` 按设计编译并运行工作树里的代码(本仓 `crates/kanzei-app/build.rs` 与两个可运行 bin
均已确认存在),而 agent 持有 `edit` 权限——**任何黑名单都关不掉这个洞,因为危险性在程序
语义里,不在 shell 语法里**;cargo 子命令白名单同样不解决。

**用户定案:接受,按方案的「不假装 + 收窄」执行。** 前提是用户信任自己跑的模型与任务源——
对自用工具这个假设成立(见 `usability-first` 的定位)。因此:

- 契约必须**明写**在 `cmdline.rs` 模块头与启动告警里:「段级闸门只保证被调起的**程序**与
  **工作目录**是操作员写下的那一组,**不保证被调起的程序自身不能执行任意代码**」。
- ACE 类程序(cargo/node/npm/python/pwsh/awk 等二十余个)命中即逐条点名告警:
  「该规则等于在 `<workdir>` 授予任意代码执行」;`workdir="*"` 时措辞升级。
- RELOCATION_FLAGS 否决表照做(它守的是「workdir 是授权身份」这条**真**红线,是可测的)。
- **残余 ACE 风险如实标注**,不得在任何文案里表述成"安全"。

> 若将来要把**不受信任的任务源**(如外部 issue)交给循环,本档不适用,须另起一档
> 「只放行真正的叶子命令」——那一档 Rust 开发跑不了,是不同的产品形态。本轮不做。

### 定案 2 · R-177 内容④(session_id 加 worktree 后缀):**接受降级**

本轮不做。替代交付:**分支名在 `ProcessInfo` 回显**(F11 顺带)。理由三条(证伪三 FATAL-2):
①F4 定死 `project_dir` 恒主根后 `#p{n}` 已保证唯一,后缀带不来唯一性收益;②改身份串撞
**D-176 红线**——既有线的会话历史会集体失联;③收益不抵 12 个调用点与 id 解析器的隐性契约。

**R-177 条目状态如实停在 `[doing]`**,并在进展里逐条列出未覆盖项(内容④)。不得因为大部分
批次绿了就翻 `[fixed]`。

### 定案 3 · 裸 `$VAR` / `$?`:**放行**

裸变量展开不算命令替换——它不能执行代码,只能展开成字符串。`$(...)` 与反引号**仍拦**。

依据是实测:D-267 被拒清单的第①条正是 `node <脚本> && echo "EXIT=$?"`,拆掉尾部 `echo`
后同一条命令直接放行——这类写法在 agent 输出里高频出现,一律拦会让无人值守体验崩掉。
两份原方案在裸 `$` 上口径不一致,**本节即为口径**:词法器只对 `$(` 、反引号、`<(` 判
Undecidable,对 `$NAME` / `$?` / `${NAME}` 放行。

> 已知代价(如实记录):`cargo test 2>&1 | Select-Object -Last 40` 因 `2>&1` 命中重定向
> **仍被拦**。重定向那一类本轮不放行,agent 需要学会拆命令。
