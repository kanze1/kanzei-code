...
**方案 A**：架构浏览作为主导航新视图（`view-arch`），数据来自新增只读 Tauri command（读取 architecture/README.md 索引与 docs/design/ 目录清单，输出带状态分组的文档树），前端复用 17-files.js 的目录树渲染模式 + 应用内 Markdown 查看器打开设计文档；记忆配置触达既有 memory_* 命令（记忆页与架构浏览同级入口，不重复造后端）。
...
- 图形化架构图若用户后续要求，另行评估（mermaid/d3 vendored 体积与 A-007 判据）。
- direction_taste.md §6 曾建议冻结 R-122（待用户确认，未落 tracker）；本报告按 R-122 仍为 todo 推进，若用户拍板冻结则本条目随动。
