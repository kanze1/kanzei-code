...
> **2026-09-26 后续修订：下文色值和绿 / 红 / 琥珀映射转为历史记录。** 用户确认的提示色为亮橙、墨蓝、电光青；中性底色保留，侧栏与输入框增加轻微静态暖灰质感。当前色值与范围见 [品牌与界面质感](brand_refresh.md)。下文关于状态文字、焦点、优先级只编码一次和对比度的约束继续有效。
...
| 琥珀 | --warn(--alert 是别名) | 需要注意 | 阻塞(非零时)、卡住、超时、未启动、中断、暂停、配置未保存、文件页未保存(头部「● 未保存」与树上的点,见 files_editor.md §4)与冲突横幅(--alert-soft 浅底)、额度、上下文告警、风险开关(总是允许 / 自动放行)、待澄清;**需要你**:权限卡与提问胶囊(--surface-attention)、子代理等待批准(data-state=attention)、待决策、研究等你选方向 |
