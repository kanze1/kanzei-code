...
| 正文 | --fg | #e5e5e5 |
| 次要文字 | --dim | #9a9a9a(Codex 标签 #727272 在弹窗上达不到 4.5:1,取能过线的最暗值) |
| 强调色填充 | --accent | #d25e28(Codex 发送键同色) |
| 强调色文字 | --accent-text | #ff8549 |
| 状态色 | --ok / --err / --warn | #5fc98c / #ff7b72 / #e8b84a |

- 顺序约束(⑥g):L(--bg) < L(--sidebar-bg) < L(--surface-raised),且 L(--code-bg) > L(--bg)。
- 主区更深之后同样的阴影更难看见,--elev-2/--elev-3/--elev-composer 各加深一档。
- 亮色基本不动(主区纯白、侧栏 #f7f7f7 浅灰,本来就是 Codex 亮色的层次),只把品牌橙统一成与暗色同一个填充值 #d25e28;文字值 #a04b1a 不变。
...
| 灰 | --dim / --fg / --fg-strong | 其余一律中性 | todo、open、draft、空闲、停止中、已停止、可执行、被取得、归档、配置来源标签、线路和角色身份、每一次工具调用成功(默认不着色,失败才红)、批次格与进度条、记忆 SOP 分类 |
| 蓝 | --info、--syntax-*、--diff-* | 语法 / 数据 | 只用于 .sv-json 键和代码着色,不表达任何状态 |

## 4. 具体规则

- **优先级**只编码一次,在行内胶囊上:P0 = --badge-err 底 + --err 字;P1 = --badge-soft 底 + --fg-strong 字;P2 = --badge-soft 底 + --dim 字;P3 与未定 = 透明底 + 1px --border 描边。没有竖条,编号不染色。胶囊底用半透明 --badge-soft:焦点卡本身是 --panel2 底,不透明 panel2 会让胶囊消失。条目上的 pri-P* 类只作钩子保留。
- **批次格**是进度不是状态:已完成格一律 var(--dim);唯一的彩色是「线在跑时的当前格」(强调色描边 + 扫光,动效分区)。
- **计数**:可执行白字(默认 --fg-strong);阻塞非零琥珀、非法非零红;值为 0 时 backlogStat 挂 is-zero,一律灰。
- **列表行与卡片不画彩色左竖条**(需求行、焦点卡、工作单元卡、记忆行、记忆候选、确认的发现、外部阻塞、归档列表)。焦点卡左边框只用线型(实线 = 运行证据 / 取得线,虚线 = 推断)表达依据,颜色中性。在做的一行由 doing 胶囊(强调色)+ 线真在跑时的呼吸点表达,不再染底。
...
- **rail 运行数徽标**:--accent-text 底 + --bg 字(7.36:1)。
...
- **彩色 emoji** 绕过调色板:⚡ 一律加 U+FE0E 变成文字字形(JS 里写 `"⚡\uFE0E"`,HTML 里写 `⚡&#xFE0E;`),从此继承 color;通知里的 ✅ 换成 ✓;附件行去掉 📎。
...
| ⑥b | .doc-item / .focus-card / .memory-row / .memory-candidate / .work-unit-card 的 border-left 不得引用 ok / warn / alert / info / err / danger / accent* | `.focus-card.blocked { border-left-color: var(--warn) }` |
| ⑥c | var(--info) / var(--badge-info) 只准出现在 .sv-json / 语法类选择器里 | `.queue-delivery { color: var(--info) }` |
| ⑥d | 中性语义选择器(todo / open / draft、可执行、被取得、依赖可做层、idle / stopping 点与字形、批次格、P1-P3 胶囊、线路代号框、人格签、来源标签、工具单步成功、归档)不得引用状态色 | `.kz-dot[data-state="idle"] { background: var(--ok) }` |
...
| ⑥g | 暗色表面 R=G=B;L(--bg) < L(--sidebar-bg) < L(--surface-raised);L(--code-bg) > L(--bg) | 侧栏改回 #181818、代码块改回 #171717 |
...
- 自测:照 selfTestSurfaceRules 的做法,每条判据喂一个反例(共 14 个),任何一条恒绿先红;锚点找不到也红。
- 基线证据:同一套判据跑本轮之前的 style.css 报 101 条(a 21、b 12、c 15、d 14、e 33、g 2、p 4),跑现行样式表 0 条。
...
**运行时守卫**(ui-runtime-smoke):阻塞 = 0 的计数带 is-zero、= 2 不带(实渲染 + 直接调 backlogStat 两路);子代理等待批准渲染出 `.sa-glyph[data-state=attention]`、字符 ⏸,主线等首个 token 仍是 waiting;人格签不带 line-accent-*;活动面板没有 .bg-dot、编排轨迹以 .bg-tool 角色名为身份;状态栏 ⚡ 为文字字形。
...
| dim | 6.31 | 5.86 | 5.59 | 5.10 | 4.96 | 4.69 |
| accent-text | 7.36 | 6.83 | 6.51 | 5.95 | 5.79 | 5.47 |
...
- 状态栏:普通 6.31,运行态 6.82;rail 徽标 7.36。
- 非文本:accent 在主区 4.54、在侧栏 4.22;白箭头在 accent 上 3.91;焦点环在主区 / 卡片 / 侧栏 6.31 / 5.59 / 5.86。

**亮色**:最低一项是 ok 在次级卡片上 4.81;dim 最低 5.21,accent-text 最低 5.34;accent 在主区 3.91、在侧栏 3.65,白箭头在 accent 上 3.91。
...
- `.metrics-round.halted` 仍有琥珀左竖条(度量页轮次行,不在 ⑥b 的列表行范围内);`.bg-child.warn/err/running` 的状态边框保留(活动面板子行,边框就是它的状态位)。
