...
2026-09-21 增量：[AUTO research](auto_research.md)（R-363）在既有探索与运行事实之上编排调研、用户选题、MVP、完整实验、分析与论文 PDF。新增本机/已登记 SSH 环境的依赖配置和 GPU 实测探测，后续运行绑定准备完成的环境；SSH 命令在指定 workdir 执行。复用原有环境策略，未接入云厂商付费资源供应。
