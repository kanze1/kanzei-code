...
- 身份：validated_design
- 状态：原则基线仍有效；执行模型由 R-317 的 Outcome/Work Unit/事件投影取代旧阶段计划
- 日期：2026-08-07
- 适用范围：桌面端、核心运行器、工具与协议层、项目追踪器、测试归档和发布流程
- 关联需求：R-080、R-083、R-084、R-085、R-086、R-087、R-088、R-089、R-090、R-091、R-092、R-093、R-317 (done)
- 关联缺陷：D-050、D-051、D-053、D-054、D-055、D-056、D-060、D-061、D-064、D-066、D-068、D-104～D-110
- 执行模型权威：R-317 的 Outcome/Work Unit/事件投影；本文原则、不变量和证据等级继续有效

> **2026-08-08 修订注记**:用户当日取消「质量先行、功能需求在阶段 1~4 完成前不启动」的排序约束(R-070/R-050/R-059 已解除阶段门槛,按普通优先级参与取活)。本文档的目标、不变量与证据体系(§3~§10)继续有效;§11 的阶段排期与 Go/No-Go 由「方向基线 `direction_taste.md` 的执行序」取代,阶段门槛仅作背景参考。G-002 已按此承接。

## 0.1 现行执行模型权威（R-317）

- Outcome 是用户可验收结果；Requirement 保留为结果/来源上下文，不再承担完整执行历史。
- Work Unit 是唯一可执行工作单元，拥有明确 scope、acceptance、verification、checkpoint、evidence 与 terminal 状态。
- 事件投影记录 claim、checkpoint、verify、evidence、complete 等执行事实；默认上下文消费结构化投影，不从旧正文的 todo/doing 文案推断当前工作。
- 本文 §6 的 Requirement/Defect 兼容状态机、§11 的阶段计划和 §14 的工程 TODO 均标为实施前原则/计划，不能覆盖 R-317 的现行执行契约。
...
### 6.4 需求与缺陷（实施前兼容模型；现行执行由 R-317 接管）
...
## 11. 实施前分阶段计划与 Go/No-Go（历史输入；当前 Work Unit 契约见 §0.1）
