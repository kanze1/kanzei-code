...
> **D7 已被 UI-0926 #3(2026-09-26)取代**:作用域选择器在用户现场把表单上的全局值整块拷进了项目文件(五个模型键一次全固定,之后改全局对该项目永远不生效)。现行做法是三个编辑面各写一层——设置页只写全局 `[models]`;「项目模型配置」弹窗逐键覆盖/恢复继承项目 `[models]`(`crates/kanzei-app/src/model_config.rs` 的 `project_models_save`,只接受五个模型键);输入框上方的模型/思考芯片只写本线(②层)。展示统一读 `model_effective`(与运行路径同一组解析函数)。
