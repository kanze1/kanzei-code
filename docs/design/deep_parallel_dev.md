...
- 状态: **已定案,门禁解除** —— §6 的 D1~D7 与补充决策 N1~N3 于 2026-08-10 由用户逐条拍板;R-050 原条已按定案结果关闭并拆成三条子需求,实施不再需要额外拍板
- 关联: R-050(原条,已关闭并拆条) R-177/R-178/R-179(拆出的三条子需求) R-030 R-086 R-115 R-133 R-136 R-138 R-141 R-171 R-173 D-096 D-168 D-170

> **2026-08-10 调度约束更新：** 用户已将多进程代理流的首要原则定为“并行查、串行写”。本文件的 worktree、显式主根、diff、恢复与模型隔离设计继续有效；涉及“多条开发线同时写”的段落属于旧候选方向，实施时受 [`parallel_read_serial_write_orchestration.md`](parallel_read_serial_write_orchestration.md) 与 R-171 约束：同一项目同时只允许一个 writer run，worktree 不构成绕过项目级单写的许可。**§6 的门禁已于 2026-08-10 随逐条定案解除**——但"worktree 不绕过项目级单写"这条约束不因定案而放松,它正是 D4 改写理由与 N3 默认值的来源。
...
- **仍未接的**:`run_prompt` 的进程归属校验仍比 `project_dir`(线上线后该字段会指向 worktree,须改按 `origin_project` 判定);`ProcessHandle.worktree_path` 仍恒为 `None`,没有任何路径让 `cwd` 真的指向 worktree;`KanzeiConfig::load_with_warnings` 仍从 `cwd` 加载,与 §3.2"配置读主根的 `.kanzei/kanzei.toml`"的配套约束不符,线上线后会读到 worktree 里的分支副本。
...
- **R-171**(项目级单 writer,已 [done]):R-050 退回原因②的归宿,见 §6.3;worktree 不构成绕过它的通道。
