...
- 日期: 2026-08-08
- 状态: **评审中** —— worktree/模型隔离的 §6 决策点未经用户定案前不动工；项目级单 writer 地基由已定调的 R-171 独立推进
- 关联: R-050(重启方案) R-030 R-086 R-115 R-136 D-096 D-168 D-170

> **2026-08-10 调度约束更新：** 用户已将多进程代理流的首要原则定为“并行查、串行写”。本文件的 worktree、显式主根、diff、恢复与模型隔离设计继续有效；涉及“多条开发线同时写”的段落属于旧候选方向，实施时受 [`parallel_read_serial_write_orchestration.md`](parallel_read_serial_write_orchestration.md) 与 R-171 约束：同一项目同时只允许一个 writer run，worktree 不构成绕过项目级单写的许可。§6 尚未定案的其余决策继续保持门禁。
