...
assert.ok(html.includes('id="work-priority-select"'), "缺少需求/缺陷优先级切换控件");
assert.match(js, /function workPriorityStorageKey\(\)/);
assert.match(js, /workPriority: selectedWorkPriority\(\)/);
assert.match(js, /requirements\.md.*defects\.md|defects\.md.*requirements\.md/);
...
assert.match(css, /#req-list \.complexity-meter[\s\S]*grid-template-columns: repeat\(3, 1fr\)/);
