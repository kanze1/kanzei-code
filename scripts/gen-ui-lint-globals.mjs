...
const defaultUiDir = path.resolve(here, "../crates/kanzei-app/ui");
const outFile = path.join(here, "ui-lint-globals.json");

/**
 * 从经典 script 的真实源码收集跨文件顶层 globals。
 * 该函数可被 eslint.config.js 直接调用，避免 lint 先消费陈旧缓存。
 */
export function collectUiGlobals(uiDir = defaultUiDir) {
  const names = new Set();
...
export function readCachedUiGlobals(file = outFile) {
  return JSON.parse(fs.readFileSync(file, "utf8"));
}

const isMain = process.argv[1] && path.resolve(process.argv[1]) === path.resolve(fileURLToPath(import.meta.url));
if (isMain) {
  const globals = collectUiGlobals();
  if (process.argv.includes("--check")) {
    const existing = readCachedUiGlobals();
