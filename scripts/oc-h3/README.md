...
`action-plan-v7.json` generated 11 full-character clips with a two-GPU profile.
The service is stopped and port 30010 is closed; recheck availability before
any launch. `bake_alpha.py` bakes RGB/alpha planes; `repair_details.py`
...
`crates/kanzei-app/ui/assets/oc/character-v6.json`. The selected GPU pair
...
The isolated deployment directory and SSH host alias are configured locally. Before deployment, replace the placeholder GPU UUIDs and NUMA node in the selected profile. Each launch rechecks the pair, NUMA affinity, memory and active processes.
...
- `launch.py`: checks the configured UUIDs, NUMA affinity, GPU memory, compute processes, and the local port before launching. `--check-only` performs the checks without loading the model.
...
- `gpu_smoke.py`: tests BF16 matrix multiplication and a two-rank NCCL all-reduce. Supply the matching local numeric GPU indices and NUMA node to `gpu_checks.sh`.
...
Copy these scripts to the deployment's `scripts/` directory and the approved `master-clean-v4.png` to `inputs/`. Edit the selected profile with that host's GPU UUIDs and NUMA node before launch.
...
export CUDA_VISIBLE_DEVICES="<numeric-gpu-indices>"
export H3_NUMA_NODE="<numa-node>"
NCCL_P2P_DISABLE=1 OMP_NUM_THREADS=4 timeout 90 numactl --cpunodebind="$H3_NUMA_NODE" --preferred="$H3_NUMA_NODE" \
...
The service binds to `127.0.0.1:30010`. Optional local access uses `ssh -N -L 30010:127.0.0.1:30010 <your-gpu-host-alias>`.
