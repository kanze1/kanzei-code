...
const output=fileURLToPath(new URL(process.argv.includes("--details")?"../output/oc-film-details/":process.argv.includes("--v7")?"../output/oc-film-v7/":process.argv.includes("--v6")?"../output/oc-film-v6/":"../output/oc-film/",import.meta.url));
...
const server=http.createServer(async(req,res)=>{
  try {
    ...
    if(exporting&&req.method==="POST"&&(url.pathname==="/__poster"||/^\/__(?:idle_)?frame\/\d{4}$/.test(url.pathname))){
      ...
      const folder=url.pathname==="/__poster"?"":url.pathname.startsWith("/__idle_")?"idle-frames":"frames";
      await mkdir(path.join(output,folder),{recursive:true});
      await writeFile(path.join(output,folder,url.pathname==="/__poster"?"poster.png":url.pathname.slice(-4)+".png"),Buffer.concat(chunks));
