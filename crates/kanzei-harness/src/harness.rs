...
impl Harness {
    ...
    pub fn resolve(&self, ctx: &ResolveCtx) -> anyhow::Result<Arc<HarnessSnapshot>> {
        ...
        if draft.deferred_tools.contains(crate::tool_search::TOOL_SEARCH) {
...
impl HarnessSnapshot {
    ...
    fn deferred_loading_enabled(&self) -> bool {
        self.draft.tools.get(crate::tool_search::TOOL_SEARCH).is_some()
            && !self
                .draft
                .permissions
...
mod tests {
    ...
    fn deferred_tools_only_hide_from_resident_specs_when_loader_is_visible() {
        ...
        draft.tools.insert(crate::tool_search::TOOL_SEARCH, test_tool("tool_search"));
        ...
        assert!(snapshot.deferred_catalog().unwrap().contains("deferred — deferred tool."));
