...
pub fn search(
    ...
    let all_deferred_names: Vec<String> =
        deferred.iter().map(|tool| tool.name().to_owned()).collect();
...
mod tests {
    ...
    fn keyword_search_ranks_name_hits_and_keeps_registration_order_for_ties() {
        ...
        assert_eq!(names(&result.selected), "process, read_output, background");
    }
    ...
    fn keyword_limit_is_clamped_and_defaults_to_five() {
        ...
        assert_eq!(
            search("process", None, &deferred, &HashSet::new())
                .selected
                .len(),
            3
        );
