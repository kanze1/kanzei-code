//! D-368 端到端回归:bash 围栏持锁窗口内,并发 memory 写入(memory_add 形态)必须稳定落住。
//!
//! 根因(D-368):D-364 修复只预锁了八个固定路径的活动文档,`.kanzei/memory/` 下的动态
//! 条目文件(M-xxx.md、inbox.md 等)创建前无法预锁——bash 围栏命令窗口内另一进程/另一
//! run 的 memory 工具合法新建/写入这些文件时,围栏 after 快照把它们当 bash 越界回滚删除,
//! 报成功却查无此条。修复:围栏窗口内加持 **memory 树锁**(锁目标 = `.kanzei/memory`
//! 目录,锁文件 = `.kanzei/memory.lock`),memory 写入口(write_entry/refresh_derived/
//! inbox/voided 账本)持同一把锁;窗口内并发写者等锁、窗口结束落盘;超过预算则明确报错。
//!
//! 本文件用**真实 BashTool 管线**(含 acquire_managed_locks → capture → enforce)与
...
/// 验收①③:真围栏形态——真实 BashTool 管线命令窗口(1.5s)内,并发真实
/// MemoryStore::add(另一线程模拟另一进程的 memory_add)必须等待后落住:条目文件与
/// INDEX.md 真在磁盘,bash 输出不得出现 [managed-files]。
...
async fn 真bash围栏窗口内并发memory_add等待后落盘不被误回滚() {
    ...
    // 桌面自举轮形态:bash 命令(1.5s)在跑,围栏持有全部托管文档写锁 + memory 树锁。
    ...
    // 等 bash 进入命令窗口(树锁已持有)再并发 memory 写者。
