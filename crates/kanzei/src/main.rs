...
/// `kz lock status` — R-181 降级后的「外部写入者可见性」入口。
///
/// 原方向(跨进程写租约 / `kz lock acquire|release`)在 R-182 定调后被推翻:
/// 源码并发由 worktree 物理隔离 + git 三方合并/merge-tree 预检承担(D-263 根治
/// 「把他人改动卷进提交」),文档侧互斥已由 R-138 `FileLock` 解决——不需要 run 级
...
fn lock_status_report(project_root: &Path, cwd: &Path) -> Vec<String> {
    let mut out = Vec::new();
