...
fn ensure_project_isolated(dir: &Path) -> bool { if dir.join(".kanzei").is_dir() { return false; } let Some(resolved) = kanzei_harness::config::discover_project_root(dir) else { return false; }; if std::fs::canonicalize(&resolved).ok() == std::fs::canonicalize(dir).ok() || root_has_data(&resolved) { return false; } std::fs::create_dir_all(dir.join(".kanzei")).is_ok() }
