...
pub(crate) async fn consolidate_memory_inbox(project_dir: String, current_episode_id: Option<i64>) {
    ...
    // D-409:分批消化——每轮取固定条数(单轮输入可控),manager 逐条
    // memory_inbox_discard 销账,未销账留箱下轮重试(幂等);不再把整个 inbox
    // (曾达 251KB/201 条)一次性塞进单轮 prompt(模型既读不完也销不完账)。
    ...
    loop {
        ...
        for role in ["primary", "fast"] {
            ...
            // D-409:失败可见——run 失败记诊断,不再 let _ 静默丢弃。
