...
pub(crate) use assembly::{assemble_run, RunAssembly, WriterLeaseTrace};
...
use crate::{
    ...
    process_session_id, record_live_trace, record_live_trace_at_path, runtime_for,
    stop_runtime_and_finalize, take_pending_ask, typed_events, with_session_id, AppState, LiveRun,
...
/// R-202 批2:run_task 轮末收尾段前半——打开 final_store 并把本轮终态落库:
/// typed writer 终态/会话状态/run.completed|run.failed 事件/episode 落库/轮末采集/
/// 记忆整理。返回重开的 SessionStore 供后续 conversation.updated 与 trace flush 使用;
/// 打开失败不阻断(错误已上报,返回 None)。
#[allow(clippy::too_many_arguments)]
fn persist_round_outcome(
