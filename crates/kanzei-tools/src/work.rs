...
fn compact_for_context(mut state: ResolvedControlState) -> ResolvedControlState {
    ...
    for item in &mut state.blocked_items {
        for reason in &mut item.block_reasons {
            *reason = output::prompt_safe_block_reason(reason);
        }
    }
...
mod context;
pub(crate) mod log;
pub use context::{resolved_control_prompt, resolved_control_prompt_of, WorkControlContext};
mod output;
mod reconcile;
...
mod tests {
    ...
    fn reconcile_output_lists_all_classes_without_fingerprints() {
        ...
        let output = output::reconciliation_output(&report, &dir);
