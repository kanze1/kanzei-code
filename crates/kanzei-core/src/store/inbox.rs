...
impl SessionStore {
    ...
    fn promote_next_steer(&self, session_id: &str) -> Result<Option<AdmittedInput>, StoreError> {
        Ok(self
...
mod tests {
    ...
    fn 中断收尾恢复空闲并原子取消未完成输入() {
        ...
        assert_eq!(
            store.get_session("ses_test").unwrap().unwrap().status,
            "idle"
        );
        let event = store
    ...
    fn 已完成的输入不会被后来的停止追认为取消() {
        ...
        assert_eq!(
            store.input_status("in_flight").unwrap().unwrap(),
            "cancelled"
        );
    ...
    fn r050_poc_停止一个会话不影响另一个会话队列() {
        ...
        store.create_session("ses_other", "C:/other", None).unwrap();
        store
            .admit_input("ses_test", "a-pending", "A", Delivery::Queue)
            .unwrap();
        store
            .admit_input("ses_other", "b-pending", "B", Delivery::Queue)
            .unwrap();
        store
            .admit_input("ses_other", "b-steer", "B steer", Delivery::Steer)
            .unwrap();
