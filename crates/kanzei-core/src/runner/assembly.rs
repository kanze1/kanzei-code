//! 装配域(R-257 B2):run_once_with_parts 的装配段——工具物化、task 注册、
//! system 分块(agent/baseline/记忆提示)、context_report 账单、prior 清洗与
//! 用户消息装载、以及全部轮级运行态(usage/停止文本/会话批准/压缩计数/校准/
//! 冗余门禁/召回)的初始化。自 drive.rs 原样迁出,零行为变更。

use std::sync::Arc;

use kanzei_harness::{AgentDef, HarnessSnapshot, Tool};
use kanzei_llm::{Message, Part, Role, ToolSpec, Usage};

use crate::runner::recall::RecallWatch;
use crate::runner::redundancy::RedundancyWatch;
use crate::runner::subagent::task_spec;
use crate::runner::{RunnerConfig, SubagentRuntime};
...
/// struct(生命周期留在调用方)。
pub(crate) struct RunOnceAssembly<'a> {
    pub(crate) tools: Vec<Arc<dyn Tool>>,
    pub(crate) specs: Vec<ToolSpec>,
    pub(crate) context_report: Vec<(String, usize)>,
    pub(crate) stable_system: Vec<String>,
    pub(crate) refreshable_baseline: String,
    pub(crate) messages: Vec<Message>,
    pub(crate) total_usage: Usage,
    pub(crate) last_input_tokens: Option<u64>,
    pub(crate) final_text: String,
    pub(crate) max_steps: u32,
    pub(crate) session_approved: std::collections::HashSet<(String, String)>,
    pub(crate) session_rules: Vec<(String, String)>,
    pub(crate) overflow_recoveries: u32,
    pub(crate) futile_compactions: u32,
    pub(crate) overflow_traces: Vec<String>,
    pub(crate) calibration: f64,
    pub(crate) redundancy: RedundancyWatch,
    pub(crate) recall: RecallWatch<'a>,
    pub(crate) step: u32,
}
