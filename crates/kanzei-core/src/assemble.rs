...
pub async fn build_route(resolved: &ResolvedModel, proxy: &ProxyConfig) -> anyhow::Result<Route> {
    ...
    if resolved.provider.auth.as_deref() == Some("codex") {
        ...
        return Ok(Route::openai_responses_at(&resolved.provider.base_url, headers));
    }
    ...
    match resolved.provider.protocol.as_str() {
        ...
        "openai" => Ok(Route::openai_at(&resolved.provider.base_url, api_key.as_deref())),
        "openai-responses" => {
            let headers = api_key
                .map(|k| vec![("authorization".to_string(), format!("Bearer {k}"))])
                .unwrap_or_default();
            Ok(Route::openai_responses_at(&resolved.provider.base_url, headers))
        }
