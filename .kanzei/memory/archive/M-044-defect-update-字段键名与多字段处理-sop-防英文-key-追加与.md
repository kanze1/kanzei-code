...
---
[D-322 恢复记录 2026-08-14] 本条曾被整条英文化改写(title 含 "S0P" 错字、description/正文改写为英文),文件名也被改成 `M-044-defect-req-s0p-field-replacement-semanti.md`。本次按 D-322 勘察结论从 `git show d4a4f08:.kanzei/memory/M-044-defect-update-字段键名与多字段处理-sop-防英文-key-追加与.md` 写回中文原文并还原原文件名;status 保持当前的 deprecated(不因恢复内容而复活条目),仅 updated 改为恢复日期。恢复由用户在交互会话授权执行。

[时效修正 2026-08-14] 上文第 3 条「游离段落一旦产生就永远删不掉」已过期:D-329 之后 tracker 提供了 `raw_lines` / `raw_delete <id> <ordinal>` 清理通道,活动区条目的游离行可按序号删除(本轮在 R-227 上实测有效)。原文保留不改,仅在此注明——这条正是本条被标 deprecated 的原因之一。
