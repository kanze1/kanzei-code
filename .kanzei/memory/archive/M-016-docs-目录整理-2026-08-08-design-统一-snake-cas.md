...
---
[D-322 恢复记录 2026-08-14] 本条曾被跨主题覆写:title/description/正文被换成「权限拒绝转交互轮」三主题缝合体(与文件名 docs-目录整理 完全不符)。本次按 D-322 勘察结论从 `git show 32cc02f:.kanzei/memory/M-016-docs-目录整理-2026-08-08-design-统一-snake-cas.md` 写回原文,逐字节对照恢复源;仅 updated 改为恢复日期,status/created/source 保持原值。恢复由用户在交互会话授权执行。
