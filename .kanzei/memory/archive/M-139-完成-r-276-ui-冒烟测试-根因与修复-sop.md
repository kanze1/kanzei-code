...
description: 何时推进 R-276：前端冒烟测试失败的修复流程
status: deprecated
created: 2026-08-17
updated: 2026-08-18
source: memory-manager
subject: R-276 前端冒烟测试修复流程
---

适用场景:frontend_check 或 ui_dom/ui_console 触发冒烟失败后，需要终端状态修正到 fixed/wontfix。同时处理文件路径缺失、bash 超时等复合问题。当缺陷已归档(is archived)且出现 cannot open error(os error 2)时
...
8. bash 执行终端命令(预期可能有 2 次失败模式:超时 kill+exit code 0)
...
(auto-deprecated: candidate 超出健康水位 24，按低价值优先清退；fingerprint=true，recurrence=0；原路径 C:\Users\kanzei\Documents\kanzei code\.kanzei\memory\M-139-r-276-推进-sop-从内存搜索到前端冒烟测试完整的修复链路.md)
