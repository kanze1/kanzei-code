...
- M-009 [sop] edit old_string not found：先读盘再按报错提示的精确内容重造 — 处理 edit 替换失败(old_string not found/whitespace mismatch)必读：先读盘重读再按报错精确匹配，别凭记忆或 bypass
...
- M-021 [sop] edit 报 old_string 匹配多处时须加上下文或显式 replace_all=true — 处理 edit 报 "old_string matches N locations ... make it unique with more context, or set replace_all=true" 时必读:先 read 重读确认目标串出现次数,优先加更多上下文让 old_string 唯一;只有确认要整批替换才设 replace_all=true,不要盲开 replace_all 误改其它位置。
- M-022 [sop] 验证 Rust 测试用 test_record 而非 bash — 任何涉及 cargo test 的动作第一步必读 — 任何涉及 cargo test / run test / 验证 Rust 测试的动作第一步必读——即使已有本条记忆、即使只差一步验证,验证通道唯一是 test_record,bash 不在候选集(不是"优先",是"唯一");若已误开 bash 跑 cargo test 且 stderr 报 exit code: 1 或任何编译警告/error,立即停手换 test_record,不要继续在 bash 里排查或为警告改代码。
...
- M-025 [sop] edit old_string/new_string identical: nothing to do — 处理 edit 替换失败（old_string new_string identical）必读：工具要求字符串必须不同才能生效
