...
- WIP 上限只数**可执行**活动项:**需求与缺陷合计只允许 1 个可执行活动项**(requirements 的 doing 与 defects 的 fixing 合并计数,共用同一个槽位;2026-08-10 用户定调:单线程下 agent 一次只推一条,界面按「当前 + 下一个」渲染,规则向 D-207 已实现的单条焦点语义收敛)。带合法「阻塞:」字段(或用户明确挂起)的 doing **不计入准入配额**——阻塞项自动让位,不得以「doing 名额已满」为由拒开新条目(2026-08-09 用户定调;实测教训:R-101 挂起 + R-148 等复查占满名额,R-153 被误拒,阻塞项把队列锁死)。防堆积兜底:含阻塞在内 doing 总数 >4 时不得再开新项,必须先收敛存量(逐条解除阻塞或按 §1.2 关闭转移)。
...
- 单轮粒度 = 一个完整条目:以做完当前这一条缺陷/需求为目标;同构批量改动(i18n、重命名、格式迁移)一轮吃掉完整类别,禁止两三处一轮的微切片;确实超出单轮容量才按验收子项分轮,并在进展里写明批次边界。批数由 agent 按条目实际工作量自定,**上限 10 批**,并写入 `批次: k/N`(2026-08-10 用户定调:不再由复杂度固定给数);超过 10 批说明条目太大,应收口关闭并把剩余开成后续条目。
...
- 本节是验证/提交节奏的**唯一权威参数表**;引擎已接管(R-157):生效参数以 kanzei.toml `[cadence]` 为准,改参数走设置页「验证与提交节奏」或直接编辑 kanzei.toml,不要再手改本节默认值——本节默认值仅为无配置时的兜底。当前兜底默认值:
  ...
  - **全量测试**(`cargo test --workspace`):默认只有两个触发点——①**复杂度中/大**的条目关闭前一次(复杂度小的条目关闭**不跑**全量,定向测试照常;未评估复杂度的先补字段再判定,不默认免测);②发版前(verify.ps1/release.ps1)。(2026-08-10 用户定调:全量只服务于中/大条目的收口)
