...
- WIP 上限只数**可执行** doing:非阻塞 doing 最多 2 个。带合法「阻塞:」字段(或用户明确挂起)的 doing **不计入准入配额**——阻塞项自动让位,不得以「doing 名额已满」为由拒开新条目(2026-08-09 用户定调;实测教训:R-101 挂起 + R-148 等复查占满名额,R-153 被误拒,阻塞项把队列锁死)。防堆积兜底:含阻塞在内 doing 总数 >4 时不得再开新项,必须先收敛存量(逐条解除阻塞或按 §1.2 关闭转移)。
