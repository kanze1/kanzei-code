...
- [identity: live_design; last_verified_commit: 27e829c1] [`preview_pane.md`](../../../docs/design/preview_pane.md)：网页预览面板与 browser 双后端——Tauri 子 webview(add_child,tauri unstable)+ WebView2 进程内 CDP(不开调试端口);B0 规则(不抢焦点、共用主 webview 环境、活性回环、隐藏时不截图、设备模式 = 边界缩放 + set_zoom、DevTools 只给面板);导航闸拒 *.localhost 子域 / file: / javascript:(对照组证明必需);127.0.0.1 token 静态服务(根内判定、片段 LRU);browser 按面板可见性与绑定线路由、结果首行标 backend、截图落 tool-images 并以 [tool-image] 行回显;权限分级(本机 / 本地文件 / 片段放行,外网 Ask);IPC 契约与剩余人工验收(UI2-0926 #8)。
