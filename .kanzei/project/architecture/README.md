...
- [identity: live_design; last_verified_commit: 35151bbe] [`files_editor.md`](../../../docs/design/files_editor.md)：文件页编辑与可拖拽伸缩——统一路径解析 resolve_in_root(词法拒绝 + 真实路径包含)、只读策略(.git / 托管 / 内部状态 / 二进制 / 超 4MB / 非 UTF-8)、BOM 与换行保真、file_stat / file_write 按内容指纹比较并交换与覆盖留证(quarantine files-overwrite)、写日志供跨树围栏吸收用户手改;前端保存 / 脏标记 / 冲突横幅(比较 / 用磁盘版本 / 覆盖)/ 外部改动轮询 / 切项目草稿 / 新建文件 / 按行定位,文件树分隔条沿用 installSplit 与 ui_layout(UI2-0926 #6)。
