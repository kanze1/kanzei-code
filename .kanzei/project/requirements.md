...
- 进展: 已实现真实需求优先/缺陷优先模式切换：控件按项目持久化，前端继续/鞭挞/无动作追加与 run_prompt 传递模式，后端 dev agent system/context 同步模式，默认缺陷优先。全量测试通过，准备发版。
- 验证: D-120；node --check + 三个 UI smoke；cargo test --workspace
