...
- 进展: 批1计划引擎及 R-276 消费契约已落地并提交：49c9af33 `R-277 B1 研究计划树与澄清闸门`；db2e92d7 `R-277 B1 接入计划审批 IPC 与研究工作台`。批2检索-阅读-反思环已落地并提交：8552c485 `R-277 B2 接入检索反思环与来源绑定`。批3大纲写作与 LaTeX 回环已落地并提交：824690c1 `R-277 B3 接入大纲写作与 LaTeX 回环`。批4 FACT 引用校验与预算旋钮已落地并提交：4959cc4f `R-277 B4 接入 FACT 引用校验与预算旋钮`。新增 `research_verify`：capture_source 按 sources.md URL 绑定抓取完整正文，verify_claims 将文献 keywords 与 source_text 全文机械匹配、摘要级不能支撑 V2/V3，并用 git show 校验代码 file:line@commit 锚点邻域；结果落 verification.json。budget_set/get 写入显式 budget.json，research_loop start 实际消费覆盖轮次/token/并发预算。ResearchProfile 已注册工具、权限和 prompt。T-1786922726155 当前 staged kanzei-tools 337 passed/1 ignored；T-1786922726154 rustfmt+完整 suite 同样通过。D-468/D-469 已 fixed。下一步：批5 tantivy 索引、symbols 同接口与断点续跑。
- observed_head: 4959cc4f6ddda666603eb56eeaedcfb5573ee1f9
- observed_worktree_hash: fnv1a64:cbf29ce484222325
- recorded_at: 1786966301381
