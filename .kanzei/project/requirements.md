...
## R-282 需求/缺陷条目卡片头部块统一:概览字段固定化(优先级/复杂度/批次/标签/依赖)+ 进展折叠,替代 slice(0,3) 顺序漂移 [doing]
- 优先级: P2
...
- 进展: 2026-08-16 完成(待提交):根因勘察定位「最前面的块」= 12-docs-pages.js buildFocusCard 的 slice(0,3)——焦点卡片概览按字段顺序取前 3 条,不同条目字段顺序不同导致头部块参差。落地:buildFocusCard 概览字段改为 FOCUS_FIELD_KEYS 固定 key 集(进展→验收→复现→内容→影响,缺失隐藏,最多 3 条),不再依赖字段顺序;进展折叠(|| 分隔首段截 160,全文走详情查看器);列表卡片(11-docs-list renderDocList)不动(行内无字段、展开详情全文)。六条前端冒烟全绿(T-1786903020)。验证:冒烟断言「焦点卡片缺批次进度格/缺只读字段」仍过(顶层字段与 doc-field 保留)。
- observed_head: d680359f40a335db5c27ca5d2e5a5a7d99068119
- observed_worktree_hash: fnv1a64:7269013b29d3458c
- recorded_at: 1786903027413
