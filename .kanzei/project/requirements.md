...
- 进展: R-085 受托管规范文档写入入口缺失暂不能闭环；按队列顺序转入下一条可执行需求 R-087，保留 R-085 doing。
...
- 进展: 本轮完成一项相关回归收口：修复 D-149 的继续文案双列/窄屏布局契约，并将 R-089 验收关键不变量加入 scripts/ui-a11y-smoke.mjs。既有生产调用链核验：topbar 直接提供发送/停止/侧栏/活动高频入口，更多菜单承载总结/复制/搜索等低频动作；sidebar 折叠状态通过 kz-sidebar-collapsed 持久化；todo/activity 在 1400px 以下转为右侧抽屉且同时打开时错位显示。node --check 与四项 UI smoke 全部通过。剩余 800/1024/1280 的真实浏览器视觉矩阵仍待 R-101 harness，不虚假标记 done。
