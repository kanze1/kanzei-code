...
- 进展: 该条曾以“conventions.md 被 ruleset 拒绝写入”自记阻塞，此判断已作废——conventions.md 此后多次成功更新（§1.1/§1.2/§1.3/§1.35/§9），写入路径见 M-005（用专用工具，不用 edit）。下一步：按验收把完成判定规则写入 conventions.md §1.2 邻接章节。
