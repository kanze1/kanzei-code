...
## R-142 前端最低配 ESLint:no-undef 防手误,无构建步骤 [doing]
...
- 状态: doing
- 进展: 2026-08-11 取活:需求优先模式自上而下扫描,R-174(待发版)/R-157(阻塞 D-235)/R-164(阻塞用户重启)/R-059(阻塞排期)/R-101(挂起)/R-135(空壳缺内容)均不可执行,本条为第一个完整可做项。目标:flat config ESLint(no-undef)+ main.js(拆解后 ui/*.js)零未定义变量 + 纳入冒烟。
