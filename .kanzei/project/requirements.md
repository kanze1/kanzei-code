...
- 进展: 本轮定位并修复 D-117：OpenAI Responses SSE 嵌套 error 的 context_length_exceeded 未被分类，导致长对话无法进入压缩重试；已补协议回归测试并通过 kanzei-llm 全量单测。
- 验证: D-117；cargo test -p kanzei-llm 33/33
