...
- 进展: 阻塞：本条处于 todo，但当前 WIP 已有多项 doing，且前序 R-093/R-085/R-083 等仍明确阻塞；按用户定调本轮不启动新前端条目。解除条件：前序 doing 收口并释放可执行槽；下一步：槽位释放后接入 docs_snapshot 与 renderDocList 的阻塞状态/原因展示。
- 阶段: 0
- 验收: ①列表有清晰阻塞标识；②详情展示阻塞原因、依据、解除条件、下一步及依赖；③阻塞条目与可执行条目可筛选；④自动后置顺序与页面显示一致；⑤缺少阻塞说明的条目有可见提示；⑥补前端运行时冒烟与数据展示回归。

- 阻塞: WIP 槽位已满，前序 doing 条目尚未释放；暂不启动。
...
- 进展: 阻塞：验收要求指定修改 .kanzei/project/conventions.md，但该策略管理文件被编辑工具拒绝写入，当前无可用专用规范更新接口。依据：edit 返回“permission denied by ruleset”。解除条件：开放该文件的专用更新接口或用户确认替代落点；下一步：保留 doing 记录阻塞，跳过本条，继续扫描后续需求。
...
- 阻塞: 本条涉及 docstore 持久化格式、消息历史配对与压缩/拒绝状态机的跨层高影响改动；按项目规范需先确认 schema/迁移回滚边界、消息重写策略及回归范围，当前用户尚未确认具体方案。依据: 验收条款同时覆盖 tracker 写回、runner 压缩重试与权限拒绝三条跨层路径。解除条件: 用户确认实施方案；下一步: 确认后拆为最小可回滚批次并先补针对性回归测试。
...
- 阻塞: 架构级改动，涉及前端事件层、会话状态容器与后端 ask 生命周期；按项目规范需先确认 session 状态机、pending ask 权威来源与事件重放边界，当前没有具体方案确认。依据: 验收要求跨会话并发 ask/done 不丢不串及切回重建。解除条件: 确认状态模型、查询契约和迁移/回滚边界；下一步: 方案确认后先落地后端 pending asks 契约与定向回归。
...
- 阻塞: 依赖 D-061 尚未完成；其 OAuth 共享凭证并发锁/原子替换属于第三方 CLI 共享文件的高影响改动，仍缺少用户确认的 Windows 锁、跨进程协作与替换方案。解除条件: D-061 方案确认并关闭；下一步: 先处理未知 content_block/限流分类的独立批次，但需确认是否允许拆分依赖范围。
...
- 阻塞: 依赖 R-085 尚未完成；完成判定执行约束尚未落地，当前不能将已有未提交的 ui-runtime-smoke.mjs 作为正式验收闭环。解除条件: R-085 明确并提交规范条款；下一步: 将冒烟脚本接入本地/CI 调用并补运行时断言。
...
- 阻塞: 依赖 R-085、R-093 均未完成：完成判定/VerificationRun 基线和质量总纲阶段 1 数据模型尚未确认；现有 test_run_record 无调用者，不能只补展示壳。依据: 本条退回原因已明确写入端为零调用者。解除条件: 上游 VerificationRun/schema 方案确认且有测试结果生产入口设计；下一步: 沿既定方案补生产端调用与 parse_test_blocks 回归。
...
- 阻塞: 依赖 R-080 尚未完成，且本条还缺实质进展/blocked 状态的判定契约；当前只能继续既有软判断，不能声称完整状态机验收。解除条件: R-080 测试记录供给链可用并确认进展/阻塞判定字段；下一步: 补状态迁移测试和防空转门禁。
...
- 阻塞: 阶段 3 功能按 R-093 质量总纲的阶段顺序暂不启动；阶段 1~2 的数据完整性、验证基础设施和完成判定尚未完成/确认。依据: R-093 明确阶段 1~4 完成前不启动功能类需求。解除条件: R-093 阶段门槛解除；下一步: 与 R-074 一并执行 800/1024/1280 布局验收。
...
- 阻塞: 阶段 3 功能按 R-093 的阶段顺序暂不启动；核心数据/验证门槛尚未完成。依据: R-093 规定阶段 1~4 完成前不启动功能类需求。解除条件: R-093 阶段门槛解除；下一步: 先确定 Markdown 安全渲染与反馈详情模型，再补 XSS/历史隔离回归。
...
- 阻塞: 阶段 3 功能按 R-093 的阶段顺序暂不启动；前端运行时验收基础设施 R-084 及其上游尚未完成。依据: R-093 规定阶段 1~4 完成前不启动功能类需求。解除条件: R-093 阶段门槛与 R-084 的运行时冒烟能力解除；下一步: 用键盘冒烟覆盖列出的全链路并收口焦点回归。
...
- 阻塞: 依赖 R-089 尚未完成，且阶段 3 功能按 R-093 的阶段顺序暂不启动。依据: 本条已明确并入 R-089 的布局治理，当前范围与响应式策略仍未确定。解除条件: R-089 阶段门槛解除并确认面板范围；下一步: 与布局批次一起补手柄可达、双击/键盘恢复和三档视觉验收。
...
- 阻塞: 依赖 D-108 尚未完成，且阶段 3 功能按 R-093 的阶段顺序暂不启动。依据: 动态文案、属性回切和缺失 key 尚未收口，不能只扩字典宣称双语完成。解除条件: D-108 修复并解除阶段门槛；下一步: 统一 key 驱动动态渲染，补缺失 key 检查与中英文快照。
...
- 阻塞: 依赖 R-085、R-093 均未完成；按钮虽可作为候选入口，但审查流程的终态/VerificationRun 反馈契约尚未确认，当前不能把存在按钮当作完整验收。解除条件: 上游质量基线与完成判定契约确认；下一步: 接入真实审查调用并回归验证启动、结果反馈和失败路径。
...
- 阻塞: 验收仅写“优化终端和工具体验、清晰呈现”，缺少具体范围、调用入口、状态模型与可验证指标；按规范在核心交互边界不清时不能自行扩大实现。依据: 当前没有明确要改的窗口/工具管理动作。解除条件: 确认目标界面、操作清单与验收指标；下一步: 方案确认后按一个完整交互链实施并补 UI 冒烟。
...
- 阻塞: 剩余验收项“同一轮互不冲突工具并行”依赖 R-086 的会话事件/权限询问路由，且需先确认副作用冲突判定与权限询问顺序方案；当前已有后台进程与 stdin 两项交付及回归，不能无方案扩 bash 参数。解除条件: R-086 契约与并行冲突规则确认；下一步: 按规则补 runner 批内并行、冲突串行化和并发回归。
...
- 阻塞: 依赖 D-114 尚未完成；当前只能依靠提示词/流程纪律，需先完成该缺陷收口并取得冗余调用统计的可比基线。解除条件: D-114 fixed 且统计入口方案明确；下一步: 轮末落库终端/edit/git/task 计数并导出连续两轮数据。
...
- 依赖: R-099 D-114
- 阻塞: 依赖 R-099 尚未完成，且条目已明确“待数据证明提示词纪律不足后再实施”；当前没有实施门槛证据。解除条件: R-099 连续数据证明仍存在可机械收敛的冗余模式；下一步: 按数据中的重复模式补非阻断提醒并进入 R-099 计数。
- 阶段: 1
...
- 阻塞: 依赖 R-086 尚未完成；真实 UI E2 需要先有按 session 路由和 pending ask 重建契约。解除条件: R-086 完成并提供可测试的事件/查询接口；下一步: 在安全启动基座中逐项补延期 E2 清单。
...
- 阻塞: 依赖 D-121 尚未修复；只读档位的失败退出码/权限语义必须先稳定，否则会把失败分析误报为完成。解除条件: D-121 fixed 并确认只读权限快照边界；下一步: 加 profile/readonly 入口、硬 deny write/edit 和 bash 替代提示回归。
...
- 阻塞: 阶段 4 且涉及 tracker 数据契约、依赖方向校验、反向链接和独立文档视图；这是后端模型与前端调用方的高影响改动，当前未确认 refs/依赖字段迁移、循环检测语义、错误级别及 UI 数据契约。依据: 验收同时要求引擎告警、详情反向链接、拓扑视图和循环依赖检测。解除条件: 确认字段 schema、迁移/回滚、校验时机和前端返回结构；下一步: 先补 tracker 只读依赖图模型与定向回归，再接 UI。
...
- refs: R-054
- 阶段: 4
- 阻塞: 阶段 4 且涉及 conventions 词表、tracker 引擎枚举校验、存量需求/缺陷批量迁移和前端双维筛选；这是高影响数据契约改动，当前未确认词表迁移映射、字段兼容/回滚与 quick capture 返回契约。依据: 验收要求引擎拒绝、存量 100% 归一、UI 筛选和自动建议同时成立。解除条件: 确认领域/类型 schema、迁移映射、兼容策略与测试范围；下一步: 先定词表与迁移清单，再分引擎门禁、批量迁移、UI 三段实现。

## R-115 补充完善设置记录功能 [todo]
- priority: P1
- 原始描述: 描述(原文):
设置记录似乎不够全面
- 复杂度: 小
- 归属: kanzei
- 验收: 确保所有配置场景下的设置记录完整覆盖，无遗漏项

- 进展: 阻塞：验收仅写“所有配置场景下完整覆盖”，没有列出配置项、遗漏范围、真实调用方或可验证覆盖矩阵，无法在不自行扩大设置数据模型的前提下实施。依据：当前条目没有字段清单和验收指标。解除条件：确认配置场景清单、记录入口及验收范围；下一步：方案明确后按完整设置链路补实现与回归。
- 阻塞: 验收范围与配置场景清单缺失，无法安全确定改动边界。

## R-116 未完成的错误需求状态被提示完成并加入活动列表 [doing]
- 复杂度: 小
- 验证: 确认未完成的要求不应被标记为已完成，也不能加入到活动中。
- 优先级: P1

- 进展: 阻塞：验收没有指出具体错误需求、误标完成的入口或“活动列表”的定义；现有 TrackerTool 状态机只保证状态流转，不知道如何判定业务上的“未完成”，直接增加 done 门禁会改变既有工作流。解除条件：提供可复现条目/入口及完成判定字段；下一步：先针对复现补定向回归，再决定后端门禁或前端提示位置。

- 阻塞: 缺少复现条目、错误入口和完成判定契约，无法安全实施。

## R-117 查询子代理开发进度悬浮面板按钮 [todo]
- 复杂度: 中
- 验收: 添加触发后弹出浮层显示最近开发和当前进展列表
- 优先级: P1
...
## R-119 支持记忆和需求工作相关导出配置 [todo]
- 复杂度: 中
- 归属: kanzei
- 验收: 可实现记忆需求等相关内容的导出功能，并覆盖系统默认及可配置项
- 优先级: P0
- 进展: 阻塞：验收只写“记忆需求等相关内容导出”，未指定导出对象范围、格式、默认项与可配置项，也未说明真实调用入口；无法在不猜测数据契约的情况下实施。解除条件：确认导出范围、格式、默认/可配置项和调用入口；下一步：按方案补导出闭环与回归。
