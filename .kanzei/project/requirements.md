...
- 优先级: P0
- 进展: 已完成标题布局与复杂度提示修复（D-035、D-037），并完成独立需求/缺陷页面筛选隔离（D-036）。当前剩余 D-032 拖拽行为、D-034 展开收纳交互，R-066 继续 doing。
- 验证: 本轮 node --check crates/kanzei-app/ui/main.js、git diff --check、cargo test -p kanzei-app 通过。
