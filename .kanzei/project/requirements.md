...
## R-334 子代理面板与活动面板改为非固定侧栏的对话式呈现 [doing]
- refs: R-281
- 优先级: P1
...
- 进展: B3 已完成：验证并收口 R-334。① Codex/Claude Code 一手资料逐项对照见 `.kanzei/research/subagent-panel-layout/prior-art.md:20-45`，保留/改变/待确认边界见 `:89-120`；②问题复现与源码锚点见同工件 `:49-87`，D-720 已登记；③实现边界与取舍已落在 `:115-120`；④ `crates/kanzei-app/ui/style.css:1180-1219` 将活动/子代理改为不参与 flex 的 absolute 临时浮层，`03-shell.js:320-328` 与 `06-activity.js:1275-1288` 保证入口互斥，`index.html:1021-1066` 保留 dialog/关闭入口和既有真实内容消费者，`scripts/ui-runtime-smoke.mjs:6800-6806`、`scripts/ui-narrow-layout-smoke.mjs:60-88` 覆盖切换互斥、主区宽度、浮层越界及多视口；T-1786922726816 六条前端冒烟与语法检查全绿。下一步：提交后运行绑定当前 HEAD 的 verify，完成 D-720 关闭和 R-334 需求关闭。
- observed_head: f8502484c0116352bf0cf2db5323624f5e6b410a
- observed_worktree_hash: fnv1a64:28a829b5c00c7e22
- recorded_at: 1787893618882

## R-335 架构图与研究绘图工具调研及 Agent 绘图工具设计 [todo]
- 优先级: P1
...
- 进展: 已登记 R-337；用户确认主维度为执行任务关闭、session 仅下钻、未关闭任务单列进行中。待 R-333 收口后按 B1 审计当前统计来源与长会话样本，再进入 B2 任务级画像设计。
- observed_head: f8502484c0116352bf0cf2db5323624f5e6b410a
- observed_worktree_hash: fnv1a64:cbf29ce484222325
- recorded_at: 1787891583502
