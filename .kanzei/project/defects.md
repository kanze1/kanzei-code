...
- 进展: 本轮最小步骤已完成但条目保持 fixing：index.html:11-18 活动栏导航全部改为原生 button，补 aria-label/aria-current 与 SVG aria-hidden；main.js:122-136 切换时同步 aria-current，保留既有 data-view→loadSettings/refreshWorkspace/refreshDocs 调用方；style.css:35-42 重置按钮默认样式并增加 :focus-visible 焦点环。验证：node --check、HTML 结构检查、git diff --check、cargo test --workspace 全部通过。缺口：项目/文档等其他 div 容器及图标按钮仍待收口，尚不能关闭。
