...
- 进展: 继续完成第五步：main.js:2017-2028 工作树 diff 长结果改写入可追溯运行日志并自动展开日志面板，仅保留短 toast 确认落点；干净工作树仍即时 toast。既有 handleWorktreeAction diff 调用方保留。验证：node --check、node scripts/ui-a11y-smoke.mjs、git diff --check、cargo test --workspace 全部通过。D-106 仍 fixing，其他长结果和失败恢复入口仍待补齐。
