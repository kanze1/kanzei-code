...
- 进展: 完成第二十一步：修复 applyLanguage 的 title/placeholder 不可逆问题，新增 I18N_ATTR_ZH 原文缓存，英文切换后再切回中文恢复原始属性文案，不再用当前英文值反推中文；ui-i18n-smoke 增加属性原文缓存契约检查。验证：node --check、node scripts/ui-i18n-smoke.mjs（14 项）、node scripts/ui-a11y-smoke.mjs、git diff --check 通过。D-108 仍 open，动态创建节点和大部分动态文案仍待迁移。
