...
- 批次: 1/2
- 进展: 批1 完成:harvest_sop 加沉淀门槛(tools<3 机械拦截)+ 候选 detail 给 manager 可照做结构模板(适用场景/步骤+判断依据/边界例外);manager_agent() prompt 加 SOP 提炼规则(纯工具罗列不算 SOP、一次性流程 NOOP);测试更新(短流程不投断言 + harvest_end_of_run 序列补 read 满足门槛)。kanzei-tools 232 全绿。批2 待做:Memory 页 sop 类条目阅读排版。
