...
- 进展: 完成第二十九步：运行状态 fallback、上下文容量条 title、上下文详情入口接入 t(key)，语言切换时同步刷新上下文入口 title。ui-i18n-smoke 扩展至 34 项契约。验证：node --check、node scripts/ui-i18n-smoke.mjs、node scripts/ui-a11y-smoke.mjs、git diff --check 通过。仍缺真实双语快照及少量启动/运行日志动态文案。
