...
- 进展: 继续完成第十步：main.js:3426-3428 移动端桥接启动失败绑定启动按钮重试；3436-3438 停止失败绑定停止按钮重试；3470-3472 设置保存失败绑定保存按钮重试。重试通过既有真实按钮调用方触发。验证：node --check、node scripts/ui-a11y-smoke.mjs（20 个静态 icon-btn）、git diff --check、cargo test --workspace 全部通过。D-106 仍 fixing，其他失败路径与真实运行验证仍待完成。
