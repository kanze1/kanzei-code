...
- 根因(两层): ①**无强制**:提交门禁(git.rs 的 fmt_gate/clippy_gate)只覆盖 Rust 侧,六条前端冒烟没有任何代码层入口,全靠 agent 自觉逐条跑;②**声称不可核**:「四条前端冒烟全过」这句话里的「四条」是自报的,没有任何机械判据比对它与 verify.ps1 清单的差集——与 compile_gate 注释里记的 2026-08-09 事故同理(「记录是 agent 自己写的……时序判据防的是『改完没重跑』,防不住『没跑却说跑了』」)。
