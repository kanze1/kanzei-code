...
- 进展: 继续完成第六步：main.js:2022-2034 对工作树 merge/discard 成功结果按 160 字符阈值分流，长结果写入日志并自动展开日志面板，短结果保留 toast。既有操作调用方保留。验证：node --check、node scripts/ui-a11y-smoke.mjs、git diff --check、cargo test --workspace 全部通过。D-106 仍 fixing，其他长结果路径及失败恢复入口仍待分流。
