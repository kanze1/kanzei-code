...
- 进展: 继续拆分完成第四步：main.js:2624-2634 工作区卡片加入 role=button、tabIndex、aria-label/current；2652-2660 增加 Enter/Space 键盘触发，沿用既有 selectWorkspaceProject 调用方；style.css:156 增加 workspace-card focus-visible。验证：node --check、git diff --check、cargo test --workspace 全部通过。D-105 仍 fixing，归档入口、图标按钮 aria-label、隐藏 checkbox 和键盘冒烟记录仍待完成。
