...
- 进展: 继续完成第十五步：main.js:2814-2817 refreshDocs 失败改用 toastError 并绑定 refreshDocs 重试；2927-2975 规范模板初始化与应用内文档读取失败分别绑定原按钮/同 kind 查看重试；3516 设置外部打开失败绑定原按钮重试。既有打开/读取调用方保留。验证：node --check、node scripts/ui-a11y-smoke.mjs（20 个静态 icon-btn）、git diff --check、cargo test --workspace 全部通过。D-106 仍 fixing，其他失败路径及真实运行验证仍待完成。
