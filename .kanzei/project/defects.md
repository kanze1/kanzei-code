...
- 进展: 继续完成设置页 agent 容器/保存操作文案一类：agent id 缺失提示、容器创建/升级/回滚结果、设置保存成功反馈均改由 t(key) 生成；动态 key 缺失检查继续通过。验证：node --check、ui-i18n/a11y/Markdown 冒烟、git diff --check 通过。仍缺版本更新、移动端停止与大量运行日志/项目操作文案，保持 open。
