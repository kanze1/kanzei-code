...
- 进展: 继续拆分完成第七步：main.js:2154-2185 为动态项目删除/重命名按钮补充包含项目名的 aria-label；style.css:586-592 将 auto-allow/auto-continue checkbox 从 display:none 改为 1px opacity 可聚焦，并增加 focus-within 焦点环。既有项目 remove/rename、checkbox change 调用方保留。验证：node --check、CSS 检查确认不再 display:none、git diff --check、cargo test --workspace 全部通过。D-105 仍 fixing，设置页动态图标按钮和仅键盘冒烟记录仍待完成。
