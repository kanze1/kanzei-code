...
- 进展: 继续拆分完成第六步：index.html 静态 icon-btn 全量补稳定 aria-label，覆盖项目初始化/添加、测试与工作树刷新、目标、历史对话、需求/缺陷/研究/规范入口、进程/工作树、日志和查看器按钮。保留 title 与既有 id 调用方。验证：node --check、静态检查确认 0 个 icon-btn 缺 aria-label、git diff --check、cargo test --workspace 全部通过。D-105 仍 fixing，动态项目删除/重命名按钮、隐藏 checkbox 和键盘冒烟记录仍待完成。
