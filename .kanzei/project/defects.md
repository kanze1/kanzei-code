...
- 进展: 完成第二十七步：语言切换时若 Provider 表格已有动态内容，自动调用 renderProviders() 重建动态控件，避免已生成的英文/中文 placeholder、title、按钮停留在旧语言。ui-i18n-smoke 扩展至 29 项契约。验证：node --check、node scripts/ui-i18n-smoke.mjs、node scripts/ui-a11y-smoke.mjs、git diff --check 通过。工作区/文档动态 DOM 仍需在语言切换时重绘，真实双语快照仍缺。
