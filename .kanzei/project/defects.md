...
- 进展: 完成第二十步：继续迁移动态运行文案，notifyRunState、活动面板标题、等待/生成/思考/工具执行/工具结果/运行状态、上下文压缩与对话总结按钮均改用 t(key)；轮次文案保留数字和中英文结构。ui-i18n-smoke 扩展至 12 项资源与动态入口契约。验证：node --check、node scripts/ui-i18n-smoke.mjs、node scripts/ui-a11y-smoke.mjs、git diff --check 通过。仍未覆盖权限弹窗、文档/工作区动态文案、属性可逆性与真实双语快照。
