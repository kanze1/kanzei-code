...
## D-267 bash 权限只有「逐条精确」与「全放行」两档:带通配的规则被无条件降级成 Ask,纯字符串规则恒不匹配 [open] (high)
- 优先级: P0
- 复杂度: 中
- 标签: 核心
- 证据等级: E1(读码自证 + 2026-08-11 三条线实测各自 3 秒内被拒停机)
- refs: R-183 R-182 D-004
- 复现: 2026-08-11 起三条 `kz run` 独立线跑任务级并行,先给每个 worktree 的 `.kanzei/kanzei.toml` 追加了按需放行规则(`action="bash"`,`resource="cargo *"` / `"node *"` 等)。三条线**全部在 3~6 秒内**以 `EXIT=3` / `stopped: permission declined` 停机,第一条 bash 就被拒。日志里被拒的资源形态是 `{"command":"git branch --show-current; git status --short","workdir":"c:/users/kanzei/documents/kz-par-b"}`。
- 根因(三层叠加,全部读码自证):
  ①`crates/kanzei-harness/src/permission.rs:198-216` `resource_match_for_action`:`action == "bash"` 时,实际请求的 value **永远是**含 `command`+`workdir` 的结构化 JSON;若 pattern 不是结构化 JSON 则 `value_is_structured && !pattern_is_structured` 命中,**直接 return false**。于是所有纯字符串 bash 规则(`"git *"`、`"cargo *"`)**恒不匹配**。项目配置里那几条 `resource = "git *"` 是死规则——实测里 `git branch` 照样被问。
  ②`permission.rs:234-236` `command_chaining_escapes`:`action == "bash" && pattern != "*" && pattern.contains('*') && !resource.is_empty()`。它**只看 pattern 里有没有 `*`**,根本不检查命令里是否真的存在 `;`/`&&`/`|` 等链接符。于是**任何带通配的 bash 规则(哪怕结构化)都被无条件降级成 Ask**,`evaluate` 在 `rule.effect == Allow` 时把它变回 `Effect::Ask`。
  ③`resource_match_for_action:200` 对 `pattern == "*"` 有前置直通,且 ② 的条件里排除了 `pattern != "*"`——**裸 `*` 是唯一不受①②约束的写法**。
- 影响(可用性,不是安全): 三层叠加的净效果是 **bash 权限只有两个可用档位:逐条逐字节精确的单命令,或者整体 `*` 全放行(yolo)**。中间没有任何可用的「按需放行」写法。直接后果:①用户配置里已累积 **12 条巨长的结构化 JSON 规则**,每条只覆盖一个具体命令、复用率为零,只会无限累积;②启动时长期存在「检测到 N 条旧 bash 权限规则；将逐次询问」的告警且无法自愈;③**`kz` 无法无人值守运行**——这是 R-183 的直接卡点,也是本次任务级并行实测停摆的原因;④用户被结构性地推向 yolo,权限系统实际上被架空。
- 边界: 不是安全漏洞——判定失败方向是**偏严**(该允许的没允许)。但「偏严到只剩 yolo」在结果上等于把用户推去关掉权限系统,所以定 P0/high。
- 修复方向(待定,勿直接照做): ②的判定应当**真的去解析命令里的链接符**(或改为按解析出的每个子命令逐个匹配规则),而不是以「pattern 含 `*`」代替;①应当允许**非结构化 pattern 匹配结构化 value 的 `command` 字段**(即 `"cargo *"` 意为「任意 workdir 下的 cargo 命令」),workdir 维度另给独立表达方式——注意这一条与 R-183 内容②「worktree 应继承主根规则」是同一个诉求的两半。
- 验收: ①`resource = "cargo *"` 形态的规则对结构化请求真实生效,有单测;②带通配的规则在命令**确实没有**链接符时不再被降级成 Ask,命令**确实有**链接符时仍降级,两个方向各有单测;③规则可覆盖「任意 workdir」与「指定 workdir」两种意图,各有单测;④修复后 `kz run` 能在 worktree 里靠一组**可手写的**规则完成一次「改代码 → cargo test → 提交」闭环(与 R-183 验收①同一条轨迹);⑤既有的 12 条结构化 JSON 规则不因本次修改而失效(向后兼容单测);⑥启动告警「N 条旧 bash 权限规则」在规则可正常匹配后消失或给出可执行的收敛路径。
