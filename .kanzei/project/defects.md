...
- 进展: 继续完成第十三步：main.js:2709-2711 工作区刷新失败改用 toastError 并绑定 refreshWorkspace 重试。既有 workspace_snapshot 调用方保留。验证：node --check、node scripts/ui-a11y-smoke.mjs（20 个静态 icon-btn）、git diff --check、cargo test --workspace 全部通过。D-106 仍 fixing，其他失败路径及真实运行验证仍待完成。
