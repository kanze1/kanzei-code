...
- 进展: 继续完成第三步：main.js:3341-3346 读取权限规则失败、3349-3357 设置读取失败、3389-3401 移动端桥接启动/停止失败均改用 toastError；移除原重复 log，保留状态回退与既有调用方。验证：node --check、node scripts/ui-a11y-smoke.mjs、git diff --check、cargo test --workspace 全部通过。D-106 仍 fixing，文档更新/权限删除/agent 容器等错误路径及重试入口仍待分流。
