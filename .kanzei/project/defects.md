...
- 进展: 继续完成第十二步：新增 deletePermissionRule(rule) 统一权限规则删除逻辑，失败时绑定同一删除操作为重试；成功后等待 loadPermissionRules 完成，避免状态刷新竞态。既有确认弹窗和按钮调用方保留。验证：node --check、node scripts/ui-a11y-smoke.mjs（20 个静态 icon-btn）、git diff --check、cargo test --workspace 全部通过。D-106 仍 fixing，其他失败路径及真实运行验证仍待完成。
