...
- 进展: 继续拆分完成第二步：main.js:2143-2149 项目条目加入 role=button、tabIndex=0、aria-label；2190-2196 增加 Enter/Space 键盘触发，沿用既有 item.click→projects_select 调用方；style.css:105 增加项目条目 focus-visible。验证：node --check、git diff --check、cargo test --workspace 全部通过。D-105 仍 fixing，文档行与其他可点击容器尚未收口。
