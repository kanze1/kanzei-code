...
- 进展: 完成第二十八步：语言切换时补齐动态页面刷新：Provider 表格重建、工作区使用 lastWorkspaceSnapshot 重绘、文档视图在当前打开时调用 refreshDocs()。ui-i18n-smoke 扩展至 32 项契约。验证：node --check、node scripts/ui-i18n-smoke.mjs、node scripts/ui-a11y-smoke.mjs、git diff --check 通过。仍缺真实浏览器双语页面/操作快照，且部分动态日志和启动状态仍含中文。
