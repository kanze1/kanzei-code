...
- 进展: 完成第二十三步：工作区卡片状态、当前对话、最近活动、排队/更新时间、归档入口标题/按钮文案接入 t(key)，用户项目名、路径、对话标题、工具活动内容仍保留原始数据。ui-i18n-smoke 扩展至 20 项资源与入口契约。验证：node --check、node scripts/ui-i18n-smoke.mjs、node scripts/ui-a11y-smoke.mjs、git diff --check 通过。仍未覆盖文档筛选/状态按钮、设置页动态文案和真实双语快照。
