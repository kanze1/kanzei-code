...
- 进展: 继续拆分完成第十三步：index.html:357 将 status-tokens 从可点击 span 改为 button，补 aria-label/aria-expanded；main.js:862-876 点击时展示 context-detail 并同步展开状态，保留 renderContextDetail 调用方；style.css:511-512 重置按钮样式并增加 focus-visible。验证：node --check、node scripts/ui-a11y-smoke.mjs、git diff --check、cargo test --workspace 全部通过。D-105 仍 fixing，活动面板剩余动态标题和真实浏览器键盘冒烟仍待完成。
