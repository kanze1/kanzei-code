...
- 证据等级: E2+E4
- 进展: 已完成最小回归：classify_provider 先识别 rate_limit_error/rate_limit/too_many_requests/overloaded_error/overloaded 等 kind，再回落消息匹配；含 token limit 的限流错误保留 Provider kind/message，不再触发上下文压缩。新增 crates/kanzei-llm/src/error.rs 两项单测，cargo test -p kanzei-llm 26 项通过。尚未实现 HTTP 429/529 的 retry-after 退避重试，故保持 fixing。
...
- 证据等级: E2
- 进展: CLI 与桌面 helper 的持久化失败路径和文本保留实现已存在；本轮执行桌面端 persist_always_allow 成功/失败回归（2 项）均通过，确认失败不返回 AlwaysAllow。完整 answer_ask UI 事件到权限响应 E2 仍待补，保持 fixing。
...
- 进展: 已完成最小权限快照修复：CLI crates/kanzei/src/main.rs:subagent_rt 与桌面 crates/kanzei-app/src/main.rs:run_task 均在 SubagentBase 后叠加 ConfigComponent，利用既有 last-match-wins 使用户 read/glob/grep 规则覆盖默认 Allow；新增 crates/kanzei-tools/src/subagent.rs::subagent_snapshot_applies_user_read_deny，验证 read */.env 被 Deny、普通路径仍 Allow。cargo test --workspace 全绿。仍缺 runner 级 task→subagent read 实际执行回归及真实 CLI/桌面 E2，暂保 fixing。
