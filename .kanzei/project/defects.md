...
- 进展: 继续完成第十一步：main.js:3341-3346 权限规则读取失败绑定 loadPermissionRules 重试；3349-3357 设置读取失败绑定 loadSettings 重试。两者沿用真实读取调用方，失败后可从日志面板恢复。验证：node --check、node scripts/ui-a11y-smoke.mjs（20 个静态 icon-btn）、git diff --check、cargo test --workspace 全部通过。D-106 仍 fixing，其他失败路径与真实运行验证仍待完成。
