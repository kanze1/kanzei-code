...
- 进展: 继续完成第四步：main.js:3402-3413 agent 容器创建/升级/回滚失败改用 toastError；3320-3328 权限规则删除失败改用 toastError；3446-3472 设置保存失败改用 toastError。既有操作调用方、表单状态和确认逻辑保留。验证：node --check、node scripts/ui-a11y-smoke.mjs、git diff --check、cargo test --workspace 全部通过。D-106 仍 fixing，剩余失败路径及重试/打开入口仍待分流。
