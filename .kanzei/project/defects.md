...
- 进展: 完成第二十四步：文档列表动态状态接入双语显示，新增 localizedDocStatus 覆盖 todo/doing/done/dropped/fixing/fixed/open/wontfix；外部阻塞徽标、复杂度徽标/选择器/保存反馈、状态流转按钮和归档入口继续使用 t(key)。用户标题、字段内容、优先级和复杂度值保持原始数据。ui-i18n-smoke 扩展至 22 项资源与入口契约。验证：node --check、node scripts/ui-i18n-smoke.mjs、node scripts/ui-a11y-smoke.mjs、git diff --check 通过。仍未覆盖设置页动态文案与真实双语快照。
