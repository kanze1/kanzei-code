...
- 进展: 2026-08-12 已完成并发布。运行态以 session_id 实时事件为先、轮询仅校准；`kz:done` 与 `kz:idle` 分离；鞭挞等待保持可停止的独立状态；profile/auto/timer 按线路隔离；run.trace 增量落库并保留轮级汇总；历史继续挂在线路下。相关测试与安装 hash 已核验，WebView2 E2 受当前探针环境限制未计入通过证据。
- 阻塞:
