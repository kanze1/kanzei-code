...
- 进展: 已完成第二步：main.js:2046-2048 工作树创建失败改用 toastError；2642-2661 工作区项目切换失败改用 toastError。两条路径现在均即时提示、写入错误日志并自动展开日志面板，既有创建/切换调用方保留。验证：node --check、node scripts/ui-a11y-smoke.mjs、git diff --check、cargo test --workspace 全部通过。D-106 仍 fixing，更多设置/文档/权限错误路径及重试入口仍待分流。
