...
- 进展: 完成第三十一步：自动推进轮间状态、自主推进等待状态、鞭挞恢复状态接入 t(key)，避免英文模式在自动续跑时出现中文状态栏。ui-i18n-smoke 扩展至 39 项契约。验证：node --check、node scripts/ui-i18n-smoke.mjs、node scripts/ui-a11y-smoke.mjs、git diff --check 通过。仍缺停止/完成通知外围文案、启动日志和真实双语操作快照。
