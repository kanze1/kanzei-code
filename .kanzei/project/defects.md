...
- 进展: 继续拆分完成第八步：main.js:3217-3222 provider 测试按钮增加 provider 级 aria-label；3247-3251 provider 删除按钮增加名称级 aria-label；3274-3278 权限规则删除按钮增加 action/resource 级 aria-label。既有设置页测试、删除调用方保留。验证：node --check、git diff --check、cargo test --workspace 全部通过。D-105 仍 fixing，键盘核心流程自动化冒烟记录仍待完成。
