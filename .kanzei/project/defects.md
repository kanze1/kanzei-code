...
- 进展: 已完成最小步骤：main.js:149-153 新增 toastError，复用既有 toast、log 和 log-panel；2031-2033 工作树 merge/discard 失败改用 toastError，因此错误即时显示后仍会留在可展开、可复制的运行日志中。既有 handleWorktreeAction/refreshWorktrees 调用方保留。验证：node --check、node scripts/ui-a11y-smoke.mjs、git diff --check、cargo test --workspace 全部通过。D-106 仍 fixing，其他错误与长结果路径及重试/打开入口仍待分流。
