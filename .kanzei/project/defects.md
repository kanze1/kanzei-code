...
- 进展: 继续拆分完成第三步：main.js:2359-2365 文档展开行加入 role=button/tabIndex/aria-label；2552-2563 增加 Enter/Space 键盘展开并同步 aria-expanded，沿用既有详情切换逻辑；style.css:560 增加文档行 focus-visible。优先级按钮、refs 按钮、复杂度 select 的既有调用方未改变。验证：node --check、git diff --check、cargo test --workspace 全部通过。D-105 仍 fixing，workspace-card、归档入口、图标按钮和键盘冒烟记录仍待完成。
