...
- 进展: 继续拆分完成第十二步：main.js:507-524 后台任务标题改为 button，补 aria-label/aria-expanded；813-843 上下文压缩纪要、对话总结标题改为 button并同步展开状态；style.css:492-493 重置 bg-title 按钮样式并增加 focus-visible。既有活动详情和摘要数据更新逻辑保留。验证：node --check、node scripts/ui-a11y-smoke.mjs、git diff --check、cargo test --workspace 全部通过。D-105 仍 fixing，活动面板剩余动态标题与真实浏览器键盘冒烟仍待完成。
