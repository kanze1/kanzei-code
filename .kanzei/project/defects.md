...
- 进展: 完成第二十二步：权限/问题弹窗的动态标题和队列状态接入 t(key)，覆盖“需要你的回答”“权限请求”“当前请求/待处理”等文案；语言切换时若弹窗已打开，会即时刷新标题和队列状态。ui-i18n-smoke 扩展至 17 项资源与入口契约。验证：node --check、node scripts/ui-i18n-smoke.mjs、node scripts/ui-a11y-smoke.mjs、git diff --check 通过。仍未覆盖动态选项外围按钮、文档/工作区文案和真实双语快照。
