...
- 进展: 继续完成第八步：index.html:342-345 新增日志面板“重试上次失败操作”按钮；main.js:142-153 toastError 支持可选 retry 回调并按需显示重试入口，255-267 执行回调且禁用期间防重复；2070-2072 工作树 merge/discard 失败绑定原操作重试。既有日志复制、操作调用方保留。验证：node --check、node scripts/ui-a11y-smoke.mjs（20 个静态 icon-btn）、git diff --check、cargo test --workspace 全部通过。D-106 仍 fixing，其他失败路径的恢复入口及真实运行验证仍待完成。
