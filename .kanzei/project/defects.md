...
## D-368 围栏窗口内 .kanzei/memory/ 动态文件并发合法写仍可能被 bash 围栏误回滚(D-364 同族残余) [fixing] (medium)
...
- 批次: 0/1
- 进展: 设计冻结:①不变量——.kanzei/memory/ 树任何写(条目/INDEX.md/inbox.md/voided-ids.md/归档搬移/index.db 重建)与 bash 围栏窗口互斥,窗口内并发 memory 写不落入窗口快照;②互斥手段——单一 memory 树锁:锁目标=memory 根目录(project=.kanzei/memory,global=~/.kanzei/memory),锁文件=同目录 <root>.lock(.kanzei/memory.lock),由围栏 acquire_managed_locks 与 memory 写入口共同持取;锁文件不在托管根镜像内且被 .kanzei/**/*.lock 忽略;③写者取不到锁(>3s 预算)明确报错不假成功;④只读检索的观测副作用 record_hits 用短预算 try-lock,围栏持锁时直接跳过(可丢可重建)。改动面:managed.rs 围栏加树锁+单测;store.rs write_entry/refresh_derived/add/append_note/clear_inbox/discard_note/void_id 持树锁、record_hits try-lock 跳过;kanzei/tests/d368_concurrent_memory_write.rs 新集成测试(真 BashTool 管线+真 MemoryStore::add)。已知残留(非本缺陷):open_db 首次建 index.db 的极窄竞态会产生 spurious [managed-files] 报告但自愈,记入缺陷关闭说明。
- observed_head: 7b06df42a2a093d690d2fcbee2b91bf4cb8c32ae
- observed_worktree_hash: fnv1a64:cbf29ce484222325
- recorded_at: 1786750487786
