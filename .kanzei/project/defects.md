...
- 进展: 完成第三十四步：发送入口的两类用户提示接入 t(key)：运行中自动鞭挞提示、未选择项目提示。ui-i18n-smoke 扩展至 46 项契约。验证：node --check、node scripts/ui-i18n-smoke.mjs、node scripts/ui-a11y-smoke.mjs、git diff --check 通过。仍缺少量普通 toast、运行日志中文和真实双语操作快照。
