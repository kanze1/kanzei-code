...
- 修复: `gh release create` 加 `--target $hash`,tag 落在真正构建的那个提交上。
...
- 修复: runner 组装 specs 后按 name+description+input_schema 的字符数追加一行 `tools/schema` 到 context_report,与 estimate_prompt_tokens 的口径一致。
