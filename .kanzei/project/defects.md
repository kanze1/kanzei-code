...
- 阻塞: 真实浏览器键盘冒烟暂缺运行环境：当前工作区无 Playwright/Chromium/Edge 命令或前端 harness；已完成的 scripts/ui-a11y-smoke.mjs 只能做静态契约检查，不能证明 E3 运行时 Tab/Enter 路径。解除条件：R-101 提供桌面端/前端 E2 harness 或可用浏览器自动化依赖。下一步：先跳过真实 E3，继续扫描下一个可做缺陷。
...
## D-106 错误与长结果普遍依赖 2.6 秒 toast,用户无法追溯、复制或恢复 [fixing] (medium)
...
- 进展: 完成第十八步：新增 reportError 统一持久错误反馈入口，错误写入可复制的运行日志并打开日志面板，不再依赖 2.6 秒 toast；toastError 改为复用该入口。已迁移进程创建/能力更新、项目移除/重命名/初始化/选择、文档排序/状态/复杂度/目标进度、快记失败、附件读取、队列撤销、启动步骤及复制失败等路径。既有重试入口与最终态刷新调用方保留。验证：node --check crates/kanzei-app/ui/main.js、node scripts/ui-a11y-smoke.mjs、git diff --check 通过。仍缺其他失败路径全面迁移及真实 UI 运行验证。
...
- 进展: 完成第二十六步：Provider 动态表单文案接入 t(key)，覆盖订阅登录态、环境变量和直接 key 的 placeholder/title、key 已设/缺失、连接测试按钮与测试中状态、context limit 不限提示；Provider 名称、协议、URL 和后端返回内容保留原始数据。ui-i18n-smoke 扩展至 28 项资源与入口契约。验证：node --check、node scripts/ui-i18n-smoke.mjs、node scripts/ui-a11y-smoke.mjs、git diff --check 通过。仍未覆盖全部设置动态文案和真实双语快照。
