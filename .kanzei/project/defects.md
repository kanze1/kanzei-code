...
- 进展: 继续完成第十六步：main.js:3201-3204 总结失败绑定 summarize-btn 重试；3004 viewer-external 外部打开失败绑定原按钮重试；均进入持久化错误日志。既有总结、查看器调用方保留。验证：node --check、node scripts/ui-a11y-smoke.mjs（20 个静态 icon-btn）、git diff --check、cargo test --workspace 全部通过。D-106 仍 fixing，其他失败路径及真实运行验证仍待完成。
