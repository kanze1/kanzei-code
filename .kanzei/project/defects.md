...
- 进展: 继续拆分完成第五步：main.js:2571-2588 将归档入口改为原生 button，补 aria-label/aria-expanded，点击时同步展开状态，双击仍沿用 openDocViewer(`${kind}-archive`)；style.css:171-172 重置按钮样式并增加 focus-visible。验证：node --check、git diff --check、cargo test --workspace 全部通过。D-105 仍 fixing，图标按钮 aria-label、隐藏 checkbox 和键盘冒烟记录仍待完成。
