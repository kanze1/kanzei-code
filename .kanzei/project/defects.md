...
- 进展: 完成第二十五步：设置页/权限设置/Provider 连通性动态反馈接入 t(key)，包括配置路径标签、删除规则按钮标题、配置读取失败、移动桥接停止状态、无 provider、连通性测试进度与完成结果。ui-i18n-smoke 扩展至 25 项资源与入口契约。验证：node --check、node scripts/ui-i18n-smoke.mjs、node scripts/ui-a11y-smoke.mjs、git diff --check 通过。仍未覆盖全部设置页动态表单文案、启动状态和真实双语快照。
