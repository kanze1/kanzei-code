...
- 进展: 继续完成第十四步：main.js:2806-2817 refreshDocs 失败改用 toastError，绑定 refreshDocs 重试并写入日志；实时 refreshDocsSoon 仍仅记录后台告警，避免高频刷新打断。既有项目文档刷新调用方保留。验证：node --check、node scripts/ui-a11y-smoke.mjs（20 个静态 icon-btn）、git diff --check、cargo test --workspace 全部通过。D-106 仍 fixing，其他失败路径及真实运行验证仍待完成。
