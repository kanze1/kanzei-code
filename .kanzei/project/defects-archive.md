...
## D-171 编号空洞:从未分配,非数据丢失 [wontfix]
- 复现: 2026-08-08 完整性门禁报 "MISSING from both active and archive — likely data loss: D-171",阻塞全部 defects 写操作;但 `git log -S "D-171"` 全历史无任何记录,该 ID 从未被分配。
- 根因: 缺陷编号在并发写入与撞车修复(814b9d6 消除 6 个编号撞车)后产生跳号,分配器未回收空洞;门禁的 ID 连续性假设把空洞误报为数据丢失。
- 影响: 无——从未存在对应缺陷条目;此空洞一度阻塞 tracker 写入。
- 修复: 补此 tombstone,使完整性门禁不再将 D-171 视为缺失。
- 验收: 完整性门禁不再报 D-171 缺失;后续缺陷编号继续递增。
- 备注: 2026-08-08 docs 整理轮补记;若未来门禁改为允许合法跳号,可删除本条目。