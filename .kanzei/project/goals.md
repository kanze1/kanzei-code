...
- 进展: 继续推进 R-066：完成 D-032。需求与缺陷现在共用按 kind 提交的拖拽排序逻辑；需求保留手动/无筛选门禁，缺陷完整列表支持拖拽，独立缺陷筛选列表避免提交不完整顺序。node --check、git diff --check、cargo test -p kanzei-app 通过。D-034 仍待后续治理。
