...
- 进展: R-059 阶段 A 再推进：补充订阅重建后恢复 persisted cursor 的边界测试，旧事件不重放且继续读取后续事件；core 26 项、app 1 项、r050-poc-check.ps1 与 git diff --check 全部通过。文档已明确 cursor_expired 与跨重启去重的生产边界。R-075 网络错误有限重试机制已入队，等待执行槽。
