...
- 核查(2026-08-07): 部分达标,验收未满足。验收要求"所有产品/功能文案"双语,实际字典仅 54 条,而 main.js 含中文字面量约 375 个(toast、日志、动态按钮、状态文案全不翻译),index.html 53 个中文 title/placeholder 仅 4 个进字典、115 个静态文本节点仅 56 个进字典,切英文必然半中半英。标题中的"关于我们"页面在 UI 中根本不存在。TreeWalker 方案对 JS 动态生成文本天然无效,需另行设计。已登记为待补范围,不重开需求。
...
- 核查(2026-08-07): 部分达标,验收未满足。验收要求"所有面板支持缩放及拖动",实际只有侧栏/当前计划/活动面板三处、且只能调宽度,完全没有位置拖动能力;对话区、设置页、权限弹窗、文档查看器、日志面板均不可调。另有质量问题:侧栏 resize 手柄 position:absolute 放在 overflow-y:auto 容器内,侧栏滚动后手柄随内容滚走够不到;无双击重置;pointerdown 未 preventDefault 会选中文本。拖拽核心逻辑(pointer capture/边界/持久化)本身正确。
...
- 核查(2026-08-07): 部分达标,验收硬指标未满足。验收要求"在 test 中标记通过",而 .kanzei/project/ 下不存在 tests.md,无任何鞭挞流程测试记录(依赖 R-080 落地)。触发修复本身真实(冷启动勾选即调度、暂停恢复轮间重调度),今日 06:03-06:16 的 20+ 条空转提交均早于修复提交 dce5a20。但防空转是软契约:硬刹车只有 steps<=1,模型调一次工具写"阻塞日记"即 steps>1 继续烧,兜底只剩连数上限;stopAutoWhenBacklogEmpty 只在需求缺陷全部关闭时停,blocked-but-open 不设防。
