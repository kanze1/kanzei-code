...
## R-332 重做记忆单项管理工作区，提升浏览、检索与编辑效率 [done]
- 发现记录: {"Intent":"让记忆单项页面更方便浏览和管理","Explicit":"登记需求并立即开始实现","Assumptions":"本条先聚焦桌面端前端信息架构与交互，不改变记忆存储格式和后端生命周期语义","Ambiguities":"是否将召回诊断面板拆成独立页面暂不影响首批实现，先采用折叠/降级展示","领域对象":"记忆工作区、记忆列表、搜索结果、记忆详情、状态筛选、排序","最小成功闭环":"用户可从分类或搜索结果定位一条记忆，明确看到当前选中项，在详情中阅读/编辑并保存后继续浏览","延后决策":"归档/墓碑替代物理删除的后端语义、批量合并与批量操作"}
- 复杂度: 中
- 来源: 用户原话：“帮我加需求把，然后直接开做”
- 标签: 前端
...
- recorded_at: 1787581446795

## R-333 记忆前端 BUG 审计与替代管理方案调研 [done]
- 先行调研: .kanzei/research/memory-frontend-management/prior-art.md
- 内容: 围绕当前记忆前端的真实问题和管理模型开展一次证据化调研：先审计现有页面、状态、事件与数据流，再对照其他记忆管理方案，形成可评审的产品/交互/数据边界建议；本条只产出审计与方案，不直接改代码。
- 发现记录: {"Intent":"调查记忆前端现有BUG并重新评估记忆管理设计","Explicit":"调查当前前端问题并对比其他管理记忆的方案","Assumptions":"当前记忆页面的主要问题既可能来自交互也可能来自数据/事件真源，需要先审计再归因","Ambiguities":"用户未指定对标产品、记忆对象模型、隐私/删除策略或最终是否实施；本轮先产出可评审方案，不擅自拍板高影响语义","领域对象":"memory 页面、记忆条目、检索/筛选/详情/整理、来源与事件投影、外部记忆管理方案","最小成功闭环":"现有BUG可复现且有源码/运行时证据，至少四个外部方案完成一手资料对比，并给出可评审的设计方向","延后决策":"最终对象模型、自动捕获范围、用户编辑/删除权、来源/权限与迁移实施顺序待用户评审"}
- 复杂度: 中
- 来源: 用户消息：「现在还有个问题，记忆的前端现在有很多BUG，设计我也不满意，你要不调查一下其他的管理这种记忆的方案的？」
- 标签: 前端
- 边界: 覆盖当前 memory 页面关键路径、记忆条目浏览/检索/筛选/详情/整理及其与后端真源的关系；不把外部方案的界面表象直接照搬，不在未确认数据保留、删除、来源和权限语义前实施迁移。
- 验收: ①当前 memory 前端关键路径的 BUG 清单有复现步骤、精确源码锚点和运行时/测试证据；②对比不少于 4 个有一手资料的外部记忆管理方案，逐项覆盖对象模型、捕获/检索、编辑/删除、来源/权限和用户反馈；③先行调研工件逐条记录出处、V0-V3、证据锚、差异与决策；④形成新的记忆前端设计建议、明确保留/改变/待用户拍板的边界，并能回指现有 BUG。
- refs: D-568
- 优先级: P1
- 批次: 2/2
- 进展: B1/B2 已完成，研究工件 .kanzei/research/memory-frontend-management/prior-art.md 已通过 prior_art validate（external_count=4、internal_count=7、valid=true）。验收逐条对账：①当前 Memory 关键路径 BUG 清单含复现步骤与源码锚点，见工件:29-78；基线失败证据见 T-1786922726812，带实验模块参数的真实运行时序列通过见 T-1786922726813；② Mem0/Zep-Graphiti/Letta/LangMem 四个官方正文方案及对象模型、捕获/检索、编辑/删除、来源/权限、反馈/审计五维矩阵见工件:87-153；③每条方案均记录出处、V1/V2、证据深度、差异与决策，机械校验见 prior_art validate valid=true；④新的三平面+证据链、入口/详情/交互不变量及保留/改变/待拍板边界见工件:156-192，并逐条回指现有 BUG。范围遵守原约束：只完成审计与方案，不改产品代码；后续删除归档、异步代际、项目切换清理、错误隔离、字段命名和 global UI 语义另立实现条目。
- observed_head: f8502484c0116352bf0cf2db5323624f5e6b410a
- observed_worktree_hash: fnv1a64:cbf29ce484222325
- recorded_at: 1787891992354

## R-334 子代理面板与活动面板改为非固定侧栏的对话式呈现 [done]
- refs: R-281
- 优先级: P1
- 内容: 审计当前子代理面板与活动面板的布局、状态来源、用户操作和信息密度；对照 Codex 与 Claude Code 的一手可核验呈现方式，形成保留/改变/待确认边界，并据此把两个面板改成不占用固定侧边栏的对话式或临时展开式体验；不改变子代理执行后端语义。
- 发现记录: {"Intent":"改善子代理面板和活动面板的视觉与空间占用","Explicit":"对照 Codex 和 Claude Code 的做法，不让两个面板占用一个侧边栏并按对照实现","Assumptions":"用户接受复用外部产品已验证的交互形态；当前问题主要在呈现层和布局，不先改子代理执行协议","Ambiguities":"未指定最终采用内联折叠、临时抽屉或独立视图；需以两家一手资料和当前代码审计后确定，不擅自拍板后端/事件语义","领域对象":"子代理面板、活动面板、主对话、工具轨迹、活动事件、侧边栏布局、响应式窗口","最小成功闭环":"两家一手资料对照可复核，当前问题有源码/运行时证据，形成不占固定侧边栏的设计并由真实调用方消费，补齐对应前端回归","延后决策":"最终内联/抽屉形态、默认展开状态、移动端呈现和历史活动保留范围待设计评审"}
- 复杂度: 中
- 来源: 用户消息：「另外子代理面板和活动面板现在也不好看，我认为这两个不用占用一个侧边栏，你看看codex和claudecode怎么做的你就怎么做就行了」
- 标签: 前端
...
## R-336 移除设置页无效使用手册，暂不实现替代引导 [done]
- 优先级: P1
- 内容: 移除设置页面现有使用手册功能，包括入口、展示内容和仅服务于该功能的死交互；保留设置页其他真实功能，不实现替代的新手引导。后续新手引导另行讨论和登记。
- 发现记录: {"Intent":"移除不符合产品引导意图且没有价值的设置页使用手册","Explicit":"设置页面的使用手册直接砍掉；本意的新手引导后续再讨论","Assumptions":"现有使用手册没有被其他关键流程依赖，移除后设置页其他能力仍应保持可达","Ambiguities":"未指定是否删除手册文案资源、测试 fixture 和历史设计文档；开工时按真实调用方/消费者审计后决定，替代引导不在本条范围","领域对象":"设置页面、使用手册入口、帮助文案、设置页布局、前端事件与冒烟断言","最小成功闭环":"设置页不再呈现使用手册入口或内容，相关死交互无调用方，其他设置功能可正常使用且前端冒烟通过","延后决策":"新手引导的入口、步骤、触发时机、可跳过/恢复语义和内容范围待后续用户讨论"}
- 复杂度: 小
- 来源: 用户消息：「设置页面的使用手册直接砍掉吧，这个没啥用，我本意是想给新手引导的，这个完全不符合我们的引导，后续我们再来讨论这个怎么做，现有的功能是完全没用」
- 标签: 前端
- 边界: 只移除设置页当前使用手册的 UI、内容和专属死交互；不改设置项、项目导出、更新检查、权限或其他帮助入口，不实现新手引导替代品。
- 验收: ①设置页不再出现使用手册入口、标题、正文或专属操作；②移除后没有遗留仅供使用手册使用的事件监听、调用方或空白布局；③设置页其他现有功能仍可达，前端语法检查与对应运行时冒烟通过；④新手引导未在本条擅自实现，并明确列为后续设计
- 进展: R-336 已完成并通过 T-1786922726824。验收对账：①已满足——设置页/对话区移除手册入口、标题、正文和专属控件，代码变更见 crates/kanzei-app/ui/index.html:684-694、:933-936；负向 DOM 断言见 scripts/ui-runtime-smoke.mjs:1662-1668。②已满足——移除项目切换刷新调用、手册读取/偏好函数、设置 listener、专属 i18n/CSS，分别见 crates/kanzei-app/ui/09-sessions.js:52-57、:806-810，crates/kanzei-app/ui/15-views-misc.js:918-920，crates/kanzei-app/ui/16-settings.js:13-16、:770-773，crates/kanzei-app/ui/style.css:806-810；函数/选择器/文件读取负向断言见 scripts/ui-runtime-smoke.mjs:1669-1676。③已满足——语言、项目导出、更新检查、提示音等其他设置控件仍在 crates/kanzei-app/ui/index.html:688-946，四模块语法检查、六条前端冒烟和窄布局冒烟均通过 T-1786922726824。④已满足——本条未实现新手引导；后续入口、步骤、触发时机、跳过/恢复语义和内容范围仍明确保留在本需求发现记录的延后决策中。
- observed_head: 23c16add951a51b0cc2dbdaf6f15a3121fddf1e5
- observed_worktree_hash: fnv1a64:e793d26957af7ba1
- recorded_at: 1787896287240

## R-337 运行画像改按执行任务关闭粒度聚合 [done]
- 优先级: P1
...
- 进展: B3 已完成并逐项对账：①当前运行画像展示字段、统计来源和会话聚合缺口：`docs/design/run_metrics_task_granularity.md:39-79,198-203` 精确锚定 EpisodeRecord/落库/run_metrics/UI 与真实 SQLite 快照，`T-1786922726829` 用可复制命令复核当前数据，`T-1786922726826` 复核真实 UI 入口加载；②执行任务身份、关闭事件、跨会话归属、未关闭任务、主指标与 session 下钻：`docs/design/run_metrics_task_granularity.md:106-152,169-178` 明确 task_id、task.started/task.closed、显式 attach、completed/in_progress、projection 真源和 round/session 下钻；③长 session 多任务按 task close 的可复核聚合：`docs/design/run_metrics_task_granularity.md:154-165` 使用真实 session episode 1154-1157，`T-1786922726828` 断言两组 task、4 rounds 与指标结果，并明确无 close 时 completed trend=0；④保留/改变/待拍板边界与后续拆分：`docs/design/run_metrics_task_granularity.md:23-27,180-189` 明确本条不改生产代码、不推断历史 close，已登记 R-338 后端事实/投影、R-339 迁移回滚、R-340 前端主视图/下钻、R-341 真实链路收口。R-337 仅完成审计与设计，未把未确认语义写入代码。
- observed_head: 0100f49f9c617ba1895994a19e11f1d60c1e7ba4
...
## R-345 实验环境登记表与运行时快照:声明与实况分离、环境漂移可见 [done]
...
- refs: R-344 R-345 T-1786922726877 T-1786922726878 T-1786922726879
- 优先级: P2
- 批次: 3/3
- 进展: 批次 3/3 已完成并通过定向验证：沿用既有 R-344 callback/事件与 R-345 环境快照链路，本批新增 `crates/kanzei-tools/src/research_runner.rs:99-139` 按环境 GPU 数、费率累计 gpu_seconds/折算金额，`198-274` 在发起前汇总探索级与环境级已用+预估成本并返回 `RESEARCH_BUDGET_EXCEEDED`，`628-638` 将预算检查置于 spawn 前，`837-870` 在终态结算成本并释放环境；`crates/kanzei-app/src/docs.rs:23-59` 从 state.db 事件和 terminal.log 生成实时可恢复快照；`crates/kanzei-app/ui/19-research.js:640-716` 渲染状态、进度条、成本、指标曲线和终端，`721-752` 在 research 视图每秒刷新。验收对账：①曲线/进度/事件风暴由 `research_runner.rs:1048-1124`、`research_runs.rs:206-304` 与 UI `19-research.js:620-638,680-703` 实现，证据 T-1786922726877/T-1786922726879；②持久事件/终端快照由 `docs.rs:23-59`、`research_runner.rs:111-139,837-870` 实现，证据 T-1786922726877/T-1786922726878；③探索/环境预算只在 `research_runner.rs:198-274,628-638` 拦新 run，告警文案明确在跑不受影响，证据 T-1786922726877；④成本字段与环境计费口径由 `research_runner.rs:99-139,591-623` 追溯到 run/environment，UI `19-research.js:695-702` 展示，证据 T-1786922726877/T-1786922726878。六项前端冒烟证据 T-1786922726879；cargo fmt --all -- --check 通过。
...
## R-350 主对话分层契约落地:工具一行可展开、轮间留白拉开、单行思考不成块 [done]
- 内容: 按 docs/design/chat_presentation_contract.md §3/§4 落地三层契约的已定部分:①工具调用默认占一行,取消无条件渲染的 `⎿ 摘要` 第二行,点击行本身展开完整详情;失败仍占一行但错误内容默认展开;②轮间留白拉到轮内的四到五倍,先摆正当前倒挂的间距(工具行 14px 反而宽于用户↔助手 12px),并清掉全仓无创建点的 .turn-divider 遗留 CSS;③单行思考不单独成块(并进正文尾部或不渲染),只保留多行可展开块。
- 发现记录: {"Intent":"让主对话区回到以正文为主,轨迹降为一行可展开,消除用户说不出来的那种密","Explicit":"工具一行可展开;轮间留白不画线;单行思考不成块;失败态默认展开为设计者假设","Assumptions":"沿用现有折叠容器与事件流,不需要后端配合;失败态默认展开这条若用户不同意可单独改","Ambiguities":"失败态是否也只占一行,本条按一行加默认展开错误内容处理","领域对象":"主对话区、工具块、思考块、轮次留白、turn-divider","最小成功闭环":"一轮含多次工具调用与思考的真实运行,主区行数显著下降且正文可一眼定位","延后决策":"轮次导航、全局详略档位、活动面板信息架构"}
- 复杂度: 中
- 来源: 用户原话「主对话的呈现和渲染现在不满意你思考一下这个点,我有点描述不清楚」;定调三条分别为「一行合理,像claude一样,但是呢,我可以点击展开」「不画线,只拉开留白」「单行不单独成块」。
- 标签: 前端
- 边界: 只改主对话区这一列的呈现,不重做活动面板信息架构;不引入 compact/verbose 全局详略开关(用户要的是默认就一行,不是给个开关);不改后端事件契约与工具产出语义;不做轮次导航;不在本条处理 markdown 结构还原(R-351)与展开天花板(R-352)。
- 验收: ①成功工具调用在主区只占一行,点击可展开详情;②失败工具调用错误内容默认可见;③轮间留白显著大于轮内,间距倒挂消失;④样式表里不再有无创建点的 .turn-divider;⑤单行思考不产生独立可点击块;⑥ui-runtime-smoke 有断言锁住「成功工具块不渲染第二行」与「单行思考不成块」。
- refs: R-342 D-725
- 优先级: P1
- 进展: 已完成并逐条核验：①成功工具结果已由 crates/kanzei-app/ui/05-chat-render.js:419-431,448-473 放入可点击 head 的同一行，详情仍按 .tool-msg-detail 展开；最终断言 scripts/ui-runtime-smoke.mjs:3694-3702 覆盖成功单行、默认收起与点击展开。②失败摘要在同一 head 行且不带 hidden，05-chat-render.js:455-457 与 smoke:3693 证明默认可见。③style.css:2030-2034 将工具间距收紧并给非首条用户消息 36px 轮间留白，smoke:3703-3705 锁定；frontend_check 通过。④已删除 style.css 原 .turn-divider 定义，smoke:3703 断言无残留。⑤05-chat-render.js:575-596,614-629 让单行 reasoning wrapper hidden、多行恢复 expandable；smoke:3706-3712 覆盖两条路径。⑥T-1786922726908 最终前端验证通过：node check、runtime 27 脚本/2346 invoke/0 错误、lint、markdown、parallel；新增单行工具/失败可见/思考块断言全部通过。i18n/a11y 的既有基线失败已单独记录为 T-1786922726907/T-1786922726905，与本条改动无关，不影响本条验收。
- observed_head: b0827f3964668a47e3091a2bc625509b43ffd65e
- observed_worktree_hash: fnv1a64:ea8f978af9e578d9
- recorded_at: 1788309110502
- 状态说明: 已满足全部验收，现标记 done。

## R-351 主对话正文结构还原:标题层级、列表缩进、斜体与段间距 [done]
- 内容: 修 04-markdown.js 的结构降级:①标题接上六级字号阶梯(现在 heading[1] 捕获了井号数量却只用 heading[2],层级当场丢弃,.md-h 不设字号不设字重,# 与 ###### 完全同形);②列表按缩进算深度(现在缩进被 \s* 吃掉,子条目全被拉平成一串平圆点);③补斜体规则(现在只有 **粗体** 一条,*斜体*/_下划线_ 符号原样显示在屏幕上);④段间距从 5px 抬到 >=0.7em(当前行高 25.8px,长回答是一堵砖)。
- 复杂度: 小
- 来源: 用户提出主对话呈现不满意后的勘察发现,锚点 04-markdown.js:93/97、style.css:1657/1664 经复核确认;正文仅占全部渲染字节的 1/72,把这零头渲染糊是双重损失。
- 标签: 前端
- 边界: 只改既有渲染器与对应样式,不换 markdown 库、不引入外部依赖;不动代码块/表格/外链的既有安全处理;不改 setRenderMarkdown 这个全仓无调用方的注入点语义。
- 验收: ①一到六级标题字号可区分且有层级感;②两层以上嵌套列表按缩进渲染出层级;③斜体与下划线不再泄漏原始符号;④段落之间留白肉眼可辨;⑤ui-markdown-smoke 覆盖上述四项。
- refs: R-350
- 优先级: P1
- 状态说明: R-351 已完成并验证；本次仅修改既有安全子集渲染器、对应样式和冒烟测试，未引入 Markdown 库或外部依赖。
- 进展: 验收逐条对账：①一到六级标题字号可区分且有层级感：04-markdown.js:98-102 按井号数量生成 h1-h6/md-h-1..6，style.css:1656-1662 用 --fs-20/18/16/14/13/12 且统一 600 字重；②两层以上嵌套列表按缩进渲染出层级：04-markdown.js:105-129 按缩进深度维护 listStack 并递归渲染 children；③斜体与下划线不再泄漏原始符号：04-markdown.js:39-42 将 *...* 与 _..._ 渲染为 em，粗体继续保留；④段落之间留白肉眼可辨：style.css:1649 将 .msg.md p margin 调为 .7em 0；⑤ui-markdown-smoke 覆盖上述四项：scripts/ui-markdown-smoke.mjs:10-26 断言六级标题、三层列表、两种斜体、字号 token 和 .7em 间距，T-1786922726920 六项前端冒烟全绿。既有表格、代码语言、安全外链/XSS 处理保持通过且未改动。
...
## R-359 取消鞭挞连续次数上限但保留轮次、上下文预算与停机条件 [done]
...
- 优先级: P1
- observed_head: f321ee34118a9cbfbd95dfa90a02a1862cc718b6
- 进展: 结项验收逐条对账：①已完成：`crates/kanzei-harness/src/auto_run.rs:308-310` 不再检查 `rounds >= max_rounds`；`auto_run.rs:842-853` 的 `旧连续次数上限不再阻断鞭挞_但轮次继续计数` 证明达到旧配置值后仍 Continue，且 `:321,:367,:378` 饱和递增。证据 T-1786922726966、T-1786922726968。②已完成：轮次计数保留；NoAction/ZeroOutput/RepeatedFailure/GoalUnreachable/StopAfterRound/AllBlocked 等独立停机边界在 `auto_run.rs:66-102,206-225` 与既有状态机回归中保留，app 端兼容状态处理在 `crates/kanzei-app/src/auto_run.rs:55-108`；证据 T-1786922726966、T-1786922726969。③已完成：旧 `auto_max/maxRounds` 仍在 `crates/kanzei-app/ui/08-auto.js:248-251,925-932` 兼容读取/保留，但不再发送为门禁（`:275-283,1039-1042`）；界面只显示轮次 `08-auto.js:176-179`、`index.html:308,360`、`20-lines.js:214-217`，旧 MaxRounds 事件在 `07-events.js:651-654` 不再报告次数硬停。证据 T-1786922726967、T-1786922726969。④已完成：取消次数与其余安全边界分别由 harness 172 项、app 256 项及前端 node --check + 六条 UI 冒烟（runtime 27 脚本/2346 invoke、lint、parallel-lines-regression、a11y、i18n、markdown）覆盖，证据 T-1786922726966~T-1786922726969。
- observed_worktree_hash: fnv1a64:a0056d2b326b71aa
- recorded_at: 1788395085983
- 验证: ① T-1786922726966/T-1786922726968；② T-1786922726966/T-1786922726969；③ T-1786922726967/T-1786922726969；④ T-1786922726966~T-1786922726969。

## R-264 前端迁移原生 ESM(勘察已完成,方案见 docs/design/ui_esm_migration.md) [done]
...
- 批次: 9/9
- 进展: 2026-09-05 关闭对账(B1-B10 已交付,剩余⑤⑥由 R-331 承接并归档 done):①已完成,scripts/ui-sources.mjs:11-21 遍历 ui/*.js 并带 MIN_UI_FILES=24 下限断言,不再解析 HTML 取清单;②已完成,scripts/ui-runtime-smoke.mjs:1317-1328 执行器分支——classic 走 vm.runInContext、ESM 走 vm.SourceTextModule 共享同一 context,逐文件 TDZ 语义保住(:1241-1423 断言),证据 T-1786922726764;③已完成,crates/kanzei-app/ui/08-compose.js:9 显式 export __kzTest,冒烟 scripts/ui-runtime-smoke.mjs:1714 经模块 namespace 取用;④已完成,逐文件迁移每单元跑六条冒烟,B10 记录 T-1786922726764(27 文件、2339 invoke、0 错误),关闭时 dist/verification.json full verify 绑定 0ac5755c 全绿;⑤已完成(由 R-331 交付),scripts/gen-ui-lint-globals.mjs、scripts/ui-lint-globals.json 及 ui-lint-smoke 清单同步校验已删除,eslint.config.js:10,90 为 sourceType module(:69 是未迁移 mobile-PWA 的既有 script 配置),证据 T-1786922726766;⑥已完成(由 R-331 交付),10 处顶层跨文件读改显式 import/setter:crates/kanzei-app/ui/01-core.js:1-29、03-shell.js:88-121、05-chat-render.js:477-556、11-docs-list.js:160-178、22-neural-flow.js:8-9;6 处 typeof 守卫改为 globalThis 存在性判断:01-core.js:308-393、02-i18n.js:1091,证据 T-1786922726766 与 scripts/ui-esm-graph.json(772 exports/198 import statements)。历史进展:B10 已完成,21-palette.js 迁移为 ESM 并导出命令面板 API(crates/kanzei-app/ui/21-palette.js:235-251),index.html:1192 改为 type=module;渐进兼容桥 01-core.js:810/02-i18n.js:1080/03-shell.js:650 已由 R-331 收口
...
## R-362 research 模式产出问题优先的论文骨架:因变量前置、主结果先行、机制走排除链、相关工作后移 [done]
...
- 发现记录: {"Intent":"让 research 模式从自己收集到的 findings 推导出文章结构,好结构是推出来的结果,不是套出来的模板","Explicit":"用户明确否掉固定模板:要自动出这个结构,而不是把这个结构绑定死","Assumptions":"findings 已带 claim/证据锚/V 等级,足以区分因变量定义、主结果、被排除候选与竞争解释;结构性质可判定而不必匹配章节名","Ambiguities":"性质违反是硬拒还是诊断、轻重课题各受哪几条性质约束,本条按只诊断不硬拒、轻课题只受三条处理","领域对象":"findings 图、write_outline 的结构依据、结构性质判据、章节与其回答的问题","最小成功闭环":"同一批真实 findings 推出满足六条性质的 outline,换个题材推出形状明显不同的 outline,两者都不来自固定章节表","延后决策":"题材分类法、章节命名建议、已归档 topic 的结构回改"}
- 复杂度: 中
- 批次: 1/1
- 来源: 2026-09-07 用户贴出一份真实论文的建议新骨架截图,原话「我说的researchmode要生成这种结构」;随后否掉了模板化实现,原话「我是要让他自动出这个结构，而不是这个结构绑定死的模板」。现状缺口:research agent 的 system 提示(crates/kanzei-tools/src/profiles.rs:112-124)只说先 write_outline 再 write_section,对文章结构零约束;write_outline 的校验(crates/kanzei-tools/src/research_write.rs:84-153)也只查 id/title/objective/source_ids 非空与 id 唯一。结构既没有推导依据也没有质量判据,完全由模型即兴决定,同一课题两次产出可以完全不同
- 标签: 后端
- 边界: 不引入固定章节表或槽位枚举,任何必须有第 N 节的写法都不属于本条;不改 V 等级、证据锚与 verify_claims 的既有契约;轻课题(只出 report.md)只受性质①③⑤约束,不要求排除链与预测表;不动 R-273 的 LaTeX 编译回环;性质校验给诊断不做硬拒,结构是判断题不是语法题;不回改已归档 topic 目录的既有结构
