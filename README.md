...
- **Outcome 是文件，执行是事件**：需求、缺陷、想法、决策、来源与发现仍是普通 Markdown；长程需求可显式拆成有界 Work Unit，执行事实追加到 SQLite 并可重建当前投影，避免历史进展反复灌入模型。
...
- **Outcome 是文件，执行是事件**：Markdown 保存人与 agent 共用的目标真源；长程执行用可认领、可 checkpoint、可逐项验证的 Work Unit 承载。
...
| `kanzei-harness` | agents、tools、skills、context sources、permissions 五类注册表与拦截器硬门禁 |
